...
@Component({
  ...
  template: `
    <div class="meal-card-root" (click)="navigateToDetail()"
      ...
      <!-- Image Area -->
      <div class="meal-card-image" style="position: relative; height: 210px; background: linear-gradient(135deg, #ecfdf5, #d1fae5); overflow: hidden;">
        @if (meal.meal_image) {
        ...
        @if (showBadge && meal.is_bestseller) {
          <span class="meal-card-badge" style="position: absolute; top: 12px; left: 12px; padding: 4px 12px; background: linear-gradient(135deg, #059669, #10b981); color: white; font-size: 11px; font-weight: 700; border-radius: 20px; box-shadow: 0 4px 12px rgba(5,150,105,0.35); letter-spacing: 0.3px; display: flex; align-items: center; gap: 4px;">
            <span style="font-size: 12px;">&#11088;</span> Bestseller
          </span>
        } @else if (showBadge && meal.is_featured) {
          <span class="meal-card-badge" style="position: absolute; top: 12px; left: 12px; padding: 4px 12px; background: linear-gradient(135deg, #2563eb, #3b82f6); color: white; font-size: 11px; font-weight: 700; border-radius: 20px; box-shadow: 0 4px 12px rgba(37,99,235,0.35); letter-spacing: 0.3px;">Featured</span>
        } @else if (showBadge && meal.is_new) {
          <span class="meal-card-badge" style="position: absolute; top: 12px; left: 12px; padding: 4px 12px; background: linear-gradient(135deg, #7c3aed, #8b5cf6); color: white; font-size: 11px; font-weight: 700; border-radius: 20px; box-shadow: 0 4px 12px rgba(124,58,237,0.35); letter-spacing: 0.3px;">New</span>
        }

        <!-- Discount badge -->
        @if (meal.has_discount) {
          <span class="meal-card-discount" style="position: absolute; top: 12px; right: 12px; padding: 4px 10px; background: linear-gradient(135deg, #dc2626, #ef4444); color: white; font-size: 11px; font-weight: 800; border-radius: 8px; box-shadow: 0 4px 12px rgba(220,38,38,0.35);">
        ...
        @if (meal.average_rating > 0) {
          <div class="meal-card-rating" style="position: absolute; bottom: 10px; left: 10px; padding: 3px 10px; background: rgba(255,255,255,0.92); backdrop-filter: blur(6px); border-radius: 20px; display: flex; align-items: center; gap: 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
        ...
        @if (meal.availability_type === 'all_day') {
          <div class="meal-card-avail" style="position: absolute; bottom: 10px; right: 10px; padding: 3px 10px; background: rgba(255,255,255,0.92); backdrop-filter: blur(6px); border-radius: 20px; display: flex; align-items: center; gap: 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
      ...
      <!-- Content -->
      <div class="meal-card-body" style="padding: 16px 16px 14px;">

        <!-- Top row: veg indicator + tags -->
        <div class="meal-card-tags" style="display: flex; align-items: center; gap: 6px; margin-bottom: 8px; flex-wrap: wrap;">
          @if (meal.dietary_type) {
            <span class="meal-card-veg" style="display: inline-flex; align-items: center; gap: 3px; padding: 2px 8px; border-radius: 4px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;
          ...
          @if (meal.category) {
            <span class="meal-card-cat" style="padding: 2px 10px; background: #f1f5f9; color: #64748b; font-size: 10px; font-weight: 500; border-radius: 20px;">{{ meal.category.name }}</span>
          }
          @if (meal.meal_type) {
            <span class="meal-card-type" style="padding: 2px 10px; background: #f0fdf4; color: #059669; font-size: 10px; font-weight: 500; border-radius: 20px;">{{ meal.meal_type.name }}</span>
          }
        </div>

        <!-- Meal Name -->
        <h3 class="meal-card-title" style="font-size: 1.05rem; font-weight: 700; color: #0f172a; margin: 0 0 4px 0; line-height: 1.35; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; transition: color 0.2s;"
        ...
        @if (meal.short_description || meal.description) {
          <p class="meal-card-desc" style="font-size: 0.8rem; color: #94a3b8; margin: 0 0 10px 0; line-height: 1.55; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
            {{ meal.short_description || meal.description }}
          </p>
        }

        <!-- Nutrition + Spice row -->
        <div class="meal-card-nutrition" style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; flex-wrap: wrap;">
          @if (meal.calories > 0) {
        ...
        <!-- Price + Actions -->
        <div class="meal-card-price-row" style="display: flex; align-items: center; justify-content: space-between; padding-top: 12px; border-top: 1px solid #f1f5f9;">
          <div class="meal-card-price-block" style="display: flex; flex-direction: column;">
            <div style="display: flex; align-items: baseline; gap: 6px;">
              <span class="meal-card-price" style="font-size: 1.25rem; font-weight: 800; color: #059669; line-height: 1;">
            ...
            @if (meal.tax_percentage > 0) {
              <span class="meal-card-gst" style="font-size: 0.65rem; color: #94a3b8; margin-top: 1px;">+{{ meal.tax_percentage }}% GST</span>
            }
          </div>
          <div class="meal-card-actions" style="display: flex; gap: 6px;" (click)="$event.preventDefault(); $event.stopPropagation()">
            <button class="meal-card-add" (click)="onAddToCart.emit(meal)"
              ...
              <span style="font-size: 14px;">+</span> <span class="meal-card-add-label">Add</span>
            </button>
            <button class="meal-card-buy" (click)="onBuyNow.emit(meal)"
  ...
  styles: [`
    @media (max-width: 639.98px) {
      ...
      .meal-card-add .meal-card-add-label {
        display: none !important;
      }
      .meal-card-add span {

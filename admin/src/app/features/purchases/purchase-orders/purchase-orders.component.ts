...
import { PurchaseOrder, PURCHASE_ORDER_STATUSES, PAYMENT_STATUSES } from '../../../core/models/purchase/purchase.model';
  ...
  template: `
    <div *ngIf="loading && orders.length === 0" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading purchase orders...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || orders.length > 0" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Purchase Orders</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Purchase Orders</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Manage purchase orders and supplier transactions</p>
        </div>
        <a routerLink="/admin/purchases/orders/create"
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px;">
        ...
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          ...
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #f3f4f6; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #6b7280;">edit_note</span>
          </div>
          <div>
        ...
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          ...
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #fef3c7; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #d97706;">swap_horiz</span>
          </div>
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Partial Received</p>
            <p style="font-size: 22px; font-weight: 800; color: #166534; margin: 0;">{{ stats?.partially_received || 0 }}</p>
          </div>
        </div>
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,0.05)';this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #f3e8ff; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #7c3aed;">inventory_2</span>
          </div>
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Received</p>
            <p style="font-size: 22px; font-weight: 800; color: #166534; margin: 0;">{{ stats?.received || 0 }}</p>
          </div>
        </div>
      </div>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
          <div style="position: relative; flex: 1; min-width: 200px;">
            ...
            <input type="text" [(ngModel)]="searchQuery" (ngModelChange)="onSearchDebounce()" placeholder="Search by PO number or supplier..."
          ...
          <select [(ngModel)]="statusFilter" (change)="onFilterChange()"
            ...
            <option *ngFor="let s of orderStatuses" [ngValue]="s.value">{{ s.label }}</option>
          </select>
          <select [(ngModel)]="paymentFilter" (change)="onFilterChange()"
            ...
            onfocus="this.style.borderColor='#059669';this.style.boxShadow='0 0 0 3px rgba(5,150,105,0.1)'" onblur="this.style.borderColor='#e5e7eb';this.style.boxShadow=''">
            <option [ngValue]="null">All Payments</option>
            <option *ngFor="let p of paymentStatuses" [ngValue]="p.value">{{ p.label }}</option>
          </select>
          <button *ngIf="searchQuery || statusFilter || paymentFilter" (click)="clearFilters()"
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden;">
        ...
        <div *ngIf="!loading && orders.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No purchase orders found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 280px; margin: 0 0 24px 0;">
            <span *ngIf="searchQuery || statusFilter || paymentFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!searchQuery && !statusFilter && !paymentFilter">No purchase orders have been created yet.</span>
          </p>
          <a *ngIf="!searchQuery && !statusFilter && !paymentFilter" routerLink="/admin/purchases/orders/create"
            ...
            onmouseover="this.style.background='#047857';this.style.transform='translateY(-1px)'" onmouseout="this.style.background='#059669';this.style.transform=''">
            <span class="material-icons" style="font-size: 18px;">add</span> Create First Order
          </a>
          <button *ngIf="searchQuery || statusFilter || paymentFilter" (click)="clearFilters()"
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                <th style="padding: 10px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 130px;">PO Number</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 110px;">Order Date</th>
            ...
            <tbody>
              <tr *ngFor="let row of orders; let i = index" (click)="viewOrder(row.uuid)" [attr.data-idx]="i"
                ...
                <td style="padding: 12px 16px;">
                  <span style="font-family: monospace; font-weight: 700; color: #047857;">{{ row.po_number }}</span>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="color: #374151;">{{ row.order_date | date:'mediumDate' }}</span>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="color: #374151;">{{ row.supplier_name || '-' }}</span>
                </td>
                <td style="padding: 12px 12px; text-align: right;">
                  <span style="font-weight: 700; color: #1f2937;">{{ row.grand_total | number:'1.2-2' }}</span>
                </td>
                <td style="padding: 12px 12px; text-align: center;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                    [style.background]="getPaymentStatusClass(row.payment_status).bg"
                    [style.color]="getPaymentStatusClass(row.payment_status).fg">
                ...
                <td style="padding: 12px 12px; text-align: center;">
                  <span style="display: inline-flex; align-items: center; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                    [style.background]="getOrderStatusClass(row.order_status).bg"
                    [style.color]="getOrderStatusClass(row.order_status).fg">
                    <span *ngIf="row.order_status === 'approved'" style="width: 6px; height: 6px; border-radius: 50%; background: currentColor; margin-right: 4px;"></span>
                    {{ row.order_status | titlecase }}
                  </span>
                </td>
                  ...
                  <div style="display: flex; align-items: center; justify-content: center; gap: 2px;">
                    ...
                    <div style="position: relative;">
                      ...
                      <div *ngIf="activeActionRow?.uuid === row.uuid" style="position: absolute; right: 0; top: 100%; z-index: 50; background: white; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 10px 40px rgba(0,0,0,0.12); min-width: 190px; padding: 6px; margin-top: 4px; animation: fadeIn 0.1s ease-out;">
                        ...
                        <button *ngIf="row.order_status === 'draft'" (click)="editOrder(row.uuid); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                          <span class="material-icons" style="font-size: 18px; color: #059669;">edit</span> Edit
                        </button>
                        <button *ngIf="row.order_status === 'draft'" (click)="approveOrder(row.uuid); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                          <span class="material-icons" style="font-size: 18px; color: #047857;">check_circle</span> Approve
                        </button>
                        <div style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                        <button *ngIf="row.order_status !== 'cancelled' && row.order_status !== 'closed'" (click)="cancelOrder(row.uuid); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
...
export class PurchaseOrdersComponent implements OnInit, OnDestroy {
  ...
  activeActionRow: PurchaseOrder | null = null;
  ...
  stats: any = null;
  orderStatuses = PURCHASE_ORDER_STATUSES;
  ...
  loadOrders(): void {
    ...
    this.purchaseApi.getPurchaseOrders(params).subscribe({
      next: (res) => {
        this.orders = res.data || [];
        this.totalCount = res.meta?.total || this.orders.length;
        this.loading = false;
      },
      error: () => { this.loading = false; this.notification.error('Failed to load purchase orders'); },
    });
  }

  loadStats(): void {
    this.purchaseApi.getPurchaseOrderStats().subscribe({
  ...
  clearSearch(): void { this.searchQuery = ''; this.currentPage = 1; this.loadOrders(); }
  ...
  toggleActionMenu(row: PurchaseOrder | null): void {
    this.activeActionRow = this.activeActionRow?.uuid === row?.uuid ? null : row;
  }

  viewOrder(uuid: string): void { this.activeActionRow = null; this.router.navigate(['/admin/purchases/orders', uuid]); }
  addOrder(): void { this.router.navigate(['/admin/purchases/orders', 'create']); }
  editOrder(uuid: string): void { this.activeActionRow = null; this.router.navigate(['/admin/purchases/orders', uuid, 'edit']); }

  approveOrder(uuid: string): void {
    if (window.confirm('Approve this purchase order?')) {
      this.purchaseApi.approvePurchaseOrder(uuid).subscribe({
        next: () => { this.notification.success('Purchase order approved'); this.loadOrders(); this.loadStats(); },
  ...
  cancelOrder(uuid: string): void {
    if (window.confirm('Cancel this purchase order?')) {
      this.purchaseApi.cancelPurchaseOrder(uuid).subscribe({
        next: () => { this.notification.success('Purchase order cancelled'); this.loadOrders(); this.loadStats(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  deleteOrder(uuid: string): void {
    if (window.confirm('Delete this purchase order? This cannot be undone.')) {
      this.purchaseApi.deletePurchaseOrder(uuid).subscribe({
        next: () => { this.notification.success('Purchase order deleted'); this.loadOrders(); this.loadStats(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

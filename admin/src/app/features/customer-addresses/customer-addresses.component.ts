    ...
    <section style="position: relative; background: linear-gradient(135deg, #059669, #047857, #166534); padding: 48px 32px 80px; overflow: hidden;">
      ...
      <div style="max-width: 1200px; margin: 0 auto; position: relative; z-index: 2;">
        <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; color: rgba(255,255,255,0.75); margin-bottom: 16px;">
          ...
          <a routerLink="/admin/customers" style="color: rgba(255,255,255,0.75); text-decoration: none;">Customers</a>
          <span style="font-size: 10px;">&#9654;</span>
          <span style="color: white; font-weight: 500;">Addresses</span>
        </div>
        <div style="display: flex; align-items: flex-start; justify-content: space-between;">
          <div>
            <h1 style="font-size: 28px; font-weight: 800; color: white; margin: 0 0 6px 0;">
              <span class="material-icons" style="font-size: 24px; vertical-align: middle; margin-right: 8px;">home</span>
              Customer Addresses
            </h1>
            <p style="font-size: 14px; color: rgba(255,255,255,0.85); margin: 0;">Manage delivery addresses for customers</p>
          </div>
          <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
            ...
            <button (click)="exportAddresses()"
              style="padding: 8px 16px; background: rgba(255,255,255,0.15); color: white; font-weight: 600; border-radius: 10px; border: 1px solid rgba(255,255,255,0.2); font-size: 13px; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"
              onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="this.style.background='rgba(255,255,255,0.15)'">
              <span class="material-icons" style="font-size: 18px;">download</span>
              Export
            </button>
            <a routerLink="/admin/customer-addresses/create"
    ...
    <section style="max-width: 1200px; margin: 24px auto 60px; padding: 0 24px;">
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 16px 20px; margin-bottom: 16px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 180px; position: relative;">
          ...
          <input [(ngModel)]="search" (keyup.enter)="loadAddresses()" (ngModelChange)="onSearchDebounce()" placeholder="Search addresses..."
        ...
        <select [(ngModel)]="defaultFilter" (change)="onFilterChange()"
          style="padding: 10px 14px; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 13px; color: #374151; outline: none; transition: all 0.2s ease; background: #f9fafb; cursor: pointer; min-width: 130px;"
          onfocus="this.style.borderColor='#059669'; this.style.boxShadow='0 0 0 3px rgba(5,150,105,0.1)'"
          onblur="this.style.borderColor='#e5e7eb'; this.style.boxShadow=''">
          <option [ngValue]="null">All Default</option>
          <option value="true">Default</option>
          <option value="false">Non-Default</option>
        </select>
      </div>

      <div *ngIf="selectedAddresses.size > 0" style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; margin-bottom: 16px; padding: 12px 20px; display: flex; align-items: center; gap: 12px; animation: fadeSlideUp 0.2s ease-out;">
        <span style="font-size: 13px; font-weight: 700; color: #047857;">{{ selectedAddresses.size }} selected</span>
      ...
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden;">
        <div *ngIf="loading" style="display: flex; align-items: center; justify-content: center; padding: 80px 20px;">
          <div style="text-align: center;">
            ...
            <p style="font-size: 13px; color: #9ca3af;">Loading addresses...</p>
          </div>
        </div>

        <div *ngIf="!loading && addresses.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 80px 20px;">
          <div style="width: 80px; height: 80px; border-radius: 20px; background: linear-gradient(135deg, #d1fae5, #a7f3d0); display: flex; align-items: center; justify-content: center; margin-bottom: 20px;">
            <span class="material-icons" style="font-size: 36px; color: #059669;">home</span>
          </div>
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No addresses found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 320px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || typeFilter || defaultFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !typeFilter && !defaultFilter">Get started by adding a customer address</span>
          </p>
          <a *ngIf="!search && !statusFilter && !typeFilter && !defaultFilter" routerLink="/admin/customer-addresses/create"
          ...
          <button *ngIf="search || statusFilter || typeFilter || defaultFilter" (click)="clearFilters()"
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              <thead>
                <tr style="background: #f0fdf4; border-bottom: 2px solid #d1fae5;">
                  ...
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em;">Address</th>
                  ...
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="xl-cell">Zone</th>
              ...
              <tbody>
                <tr *ngFor="let row of addresses" (click)="viewAddress(row)"
                  ...
                  <td style="padding: 14px 16px;" (click)="$event.stopPropagation()">
                    <input type="checkbox" (change)="toggleRow(row)" [checked]="selectedAddresses.has(row)"
                      style="width: 16px; height: 16px; cursor: pointer; accent-color: #059669; border-radius: 4px;" />
                  </td>
                  <td style="padding: 14px 16px;">
                    <div>
                      <p style="font-weight: 600; color: #166534; margin: 0; line-height: 1.3;">{{ row.customer?.full_name || '-' }}</p>
                      <p style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0;">{{ row.customer?.email || '' }}</p>
                    </div>
                  </td>
                  <td style="padding: 14px 16px;">
                    <div>
                      <div style="display: flex; align-items: center; gap: 4px; flex-wrap: wrap;">
                        <span style="display: inline-flex; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; text-transform: uppercase;"
                          [style.background]="getTypeBg(row.address_type)" [style.color]="getTypeColor(row.address_type)">{{ row.address_type }}</span>
                        <span *ngIf="row.is_default" style="padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; background: #d1fae5; color: #047857; text-transform: uppercase;">Default</span>
                        <span *ngIf="row.is_verified" class="material-icons" style="font-size: 14px; color: #059669;">verified</span>
                      </div>
                      <p style="font-size: 12px; color: #6b7280; margin: 4px 0 0 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 280px;">{{ row.full_address || row.address_line_1 || '-' }}</p>
                      <p *ngIf="row.contact_person" style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0;">{{ row.contact_person }}{{ row.contact_mobile ? ' · ' + row.contact_mobile : '' }}</p>
                    </div>
                  </td>
                  <td style="padding: 14px 16px; color: #6b7280; display: none;" class="lg-cell">{{ row.city?.name || '-' }}</td>
                  <td style="padding: 14px 16px; color: #6b7280; display: none;" class="xl-cell">{{ row.delivery_zone?.zone_name || '-' }}</td>
                  ...
                  <td style="padding: 14px 16px; text-align: center; position: relative;" (click)="$event.stopPropagation()">
                    ...
                    <div *ngIf="openMenuId === row.id"
                      style="position: absolute; right: 8px; top: 100%; background: white; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 8px 24px rgba(0,0,0,0.12); min-width: 200px; z-index: 20; padding: 6px; animation: fadeSlideUp 0.15s ease-out;">
                      <button (click)="viewAddress(row)"
                      ...
                      <button *ngIf="!row.is_verified" (click)="verifyAddress(row)"
                        style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; transition: background 0.1s ease; text-align: left;"
                        onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background=''">
                        <span class="material-icons" style="font-size: 18px; color: #059669;">verified</span> Verify
                      </button>
                      ...
                      <div style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                      <button (click)="deleteAddress(row)"
      ...
      <div *ngIf="importing" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 50; animation: fadeIn 0.2s;">
        <div style="background: white; border-radius: 20px; padding: 32px; text-align: center; box-shadow: 0 24px 48px rgba(0,0,0,0.2); max-width: 320px; width: calc(100% - 32px);">
          ...
          <p style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">Importing addresses...</p>
        ...
        <div (click)="$event.stopPropagation()" style="background: white; border-radius: 20px; padding: 24px; max-width: 480px; width: calc(100% - 32px); box-shadow: 0 24px 48px rgba(0,0,0,0.2);">
          ...
          <div style="margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; margin-bottom: 8px;">
              ...
              <span style="color: #374151;">{{ importResult.successes }} addresses imported successfully</span>
            </div>
...
export class CustomerAddressesComponent implements OnInit {
  ...
  selectedAddresses = new Set<CustomerAddress>();
  openMenuId: number | null = null;

  loading = false;
  importing = false;
  importResult: { successes: number; failures: any[]; total: number } | null = null;
  ...
  loadAddresses(): void {
    ...
    if (this.typeFilter) params['address_type'] = this.typeFilter;
    if (this.defaultFilter) params['is_default'] = this.defaultFilter;

    this.addressApi.getPaginated(params).subscribe({
      next: (res) => {
        this.addresses = res.data || [];
        this.totalCount = res.meta?.total || this.addresses.length;
        this.totalPages = Math.ceil(this.totalCount / this.perPage) || 1;
        this.activeCount = this.addresses.filter(a => a.status === 'active').length;
        this.verifiedCount = this.addresses.filter(a => a.is_verified).length;
        this.defaultCount = this.addresses.filter(a => a.is_default).length;
        this.updateStatCards();
        this.loading = false;
      },
      error: () => { this.loading = false; this.notification.error('Failed to load addresses'); },
    });
  }

  updateStatCards(): void {
    this.statCards = [
      { label: 'Total Addresses', value: this.totalCount, icon: 'home', bg: '#d1fae5', color: '#047857' },
      { label: 'Active', value: this.activeCount, icon: 'check_circle', bg: '#a7f3d0', color: '#059669' },
      { label: 'Verified', value: this.verifiedCount, icon: 'verified', bg: '#ccfbf1', color: '#0d9488' },
      { label: 'Default Set', value: this.defaultCount, icon: 'star', bg: '#fef3c7', color: '#d97706' },
    ];
  }

  onSearchDebounce(): void {
    clearTimeout(this.searchTimeout);
    this.searchTimeout = setTimeout(() => { this.currentPage = 1; this.loadAddresses(); }, 400);
  }

  clearSearch(): void { this.search = ''; this.currentPage = 1; this.loadAddresses(); }
  onFilterChange(): void { this.currentPage = 1; this.loadAddresses(); }
  clearFilters(): void {
    this.search = ''; this.statusFilter = null; this.typeFilter = null; this.defaultFilter = null;
    this.currentPage = 1; this.loadAddresses();
  }
  ...
  isAllSelected(): boolean {
    return this.addresses.length > 0 && this.selectedAddresses.size === this.addresses.length;
  }

  toggleAllRows(event: any): void {
    if (event.target.checked) {
      this.addresses.forEach(a => this.selectedAddresses.add(a));
    } else {
  ...
  toggleRow(row: CustomerAddress): void {
    if (this.selectedAddresses.has(row)) {
      this.selectedAddresses.delete(row);
    } else {
      this.selectedAddresses.add(row);
    }
    this.openMenuId = null;
  }

  clearSelection(): void { this.selectedAddresses.clear(); }

  toggleMenu(row: CustomerAddress): void {
  ...
  viewAddress(row: CustomerAddress): void { this.router.navigate(['/admin/customer-addresses', row.uuid]); }
  editAddress(row: CustomerAddress): void { this.router.navigate(['/admin/customer-addresses', row.uuid, 'edit']); }

  setDefault(row: CustomerAddress): void {
    this.openMenuId = null;
    this.addressApi.setDefault(row.uuid).subscribe({
      next: () => { this.notification.success('Default address updated'); this.loadAddresses(); },
      error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
    });
  }

  verifyAddress(row: CustomerAddress): void {
    this.openMenuId = null;
    this.addressApi.verify(row.uuid).subscribe({
      next: () => { this.notification.success('Address verified'); this.loadAddresses(); },
      error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
    });
  }

  setStatus(row: CustomerAddress, status: string): void {
    this.openMenuId = null;
    const label = status === 'active' ? 'Activate' : 'Deactivate';
    if (window.confirm(`${label} this address?`)) {
      this.addressApi.setStatus(row.uuid, status).subscribe({
        next: () => { this.notification.success(`Address ${status === 'active' ? 'activated' : 'deactivated'}`); this.loadAddresses(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  deleteAddress(row: CustomerAddress): void {
    this.openMenuId = null;
    if (window.confirm('Delete this address? This action cannot be undone.')) {
      this.addressApi.delete(row.uuid).subscribe({
        next: () => { this.notification.success('Address deleted'); this.loadAddresses(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetActive(): void {
    const ids = [...this.selectedAddresses].map(a => a.id);
    if (window.confirm(`Activate ${ids.length} selected addresses?`)) {
      this.addressApi.bulkSetStatus(ids, 'active').subscribe({
        next: () => { this.notification.success('Addresses activated'); this.clearSelection(); this.loadAddresses(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetInactive(): void {
    const ids = [...this.selectedAddresses].map(a => a.id);
    if (window.confirm(`Deactivate ${ids.length} selected addresses?`)) {
      this.addressApi.bulkSetStatus(ids, 'inactive').subscribe({
        next: () => { this.notification.success('Addresses deactivated'); this.clearSelection(); this.loadAddresses(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkDelete(): void {
    const ids = [...this.selectedAddresses].map(a => a.id);
    if (window.confirm(`Delete ${ids.length} selected addresses? This cannot be undone.`)) {
      this.addressApi.bulkDelete(ids).subscribe({
        next: () => { this.notification.success('Addresses deleted'); this.clearSelection(); this.loadAddresses(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  onFileSelected(event: Event): void {
    ...
    this.importing = true;
    this.addressApi.import(file).subscribe({
      next: (res) => { this.importing = false; this.importResult = res.data || null; this.loadAddresses(); input.value = ''; },
  ...
  exportAddresses(): void {
    ...
    if (this.typeFilter) filters['address_type'] = this.typeFilter;
    this.addressApi.export(filters).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'customer-addresses-export.csv'; a.click();

...
import { Area, AreaImportResult } from '../../core/models/master/area.model';
...
@Component({
  ...
  template: `
    <section style="position: relative; background: linear-gradient(135deg, #059669, #047857, #166534); padding: 48px 32px 80px; overflow: hidden;">
      ...
      <div style="max-width: 1200px; margin: 0 auto; position: relative; z-index: 2;">
        <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; color: rgba(255,255,255,0.75); margin-bottom: 16px;">
          ...
          <span style="color: white; font-weight: 500;">Areas</span>
        </div>
        <div style="display: flex; align-items: flex-start; justify-content: space-between;">
          <div>
            <h1 style="font-size: 28px; font-weight: 800; color: white; margin: 0 0 6px 0;">
              <span class="material-icons" style="font-size: 24px; vertical-align: middle; margin-right: 8px;">place</span>
              Areas
            </h1>
            <p style="font-size: 14px; color: rgba(255,255,255,0.85); margin: 0;">Manage all delivery areas and zones</p>
          </div>
          <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
            ...
            <button (click)="exportAreas()"
              style="padding: 8px 16px; background: rgba(255,255,255,0.15); color: white; font-weight: 600; border-radius: 10px; border: 1px solid rgba(255,255,255,0.2); font-size: 13px; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"
              onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="this.style.background='rgba(255,255,255,0.15)'">
              <span class="material-icons" style="font-size: 18px;">download</span>
              Export
            </button>
            <a routerLink="/admin/areas/create"
    ...
    <section style="max-width: 1200px; margin: -40px auto 0; padding: 0 24px; position: relative; z-index: 3; display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px;">
    ...
    <section style="max-width: 1200px; margin: 24px auto 60px; padding: 0 24px;">
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 16px 20px; margin-bottom: 16px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 180px; position: relative;">
          ...
          <input [(ngModel)]="search" (keyup.enter)="loadAreas()" (ngModelChange)="onSearchDebounce()" placeholder="Search areas..."
      ...
      <div *ngIf="selectedAreas.size > 0" style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; margin-bottom: 16px; padding: 12px 20px; display: flex; align-items: center; gap: 12px; animation: fadeSlideUp 0.2s ease-out;">
        <span style="font-size: 13px; font-weight: 700; color: #047857;">{{ selectedAreas.size }} selected</span>
        ...
        <button (click)="bulkEnableService()"
          style="padding: 6px 12px; background: #d1fae5; color: #047857; font-weight: 600; border-radius: 8px; border: none; font-size: 12px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; transition: all 0.15s ease;"
          onmouseover="this.style.background='#a7f3d0'" onmouseout="this.style.background='#d1fae5'">
          <span class="material-icons" style="font-size: 14px;">local_shipping</span> Enable Service
        </button>
        <button (click)="bulkDisableService()"
          ...
          <span class="material-icons" style="font-size: 14px;">block</span> Disable Service
        </button>
      ...
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden;">
        <div *ngIf="loading" style="display: flex; align-items: center; justify-content: center; padding: 80px 20px;">
          <div style="text-align: center;">
            ...
            <p style="font-size: 13px; color: #9ca3af;">Loading areas...</p>
          </div>
        </div>

        <div *ngIf="!loading && areas.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 80px 20px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No areas found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 320px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || countryFilter || stateFilter || cityFilter || serviceableFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !countryFilter && !stateFilter && !cityFilter && !serviceableFilter">Get started by adding your first area</span>
          </p>
          <a *ngIf="!search && !statusFilter && !countryFilter && !stateFilter && !cityFilter && !serviceableFilter" routerLink="/admin/areas/create"
            ...
            <span class="material-icons" style="font-size: 18px;">add</span> Add Area
          </a>
          <button *ngIf="search || statusFilter || countryFilter || stateFilter || cityFilter || serviceableFilter" (click)="clearFilters()"
        ...
        <div *ngIf="!loading && areas.length > 0">
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              <thead>
                <tr style="background: #f0fdf4; border-bottom: 2px solid #d1fae5;">
                  ...
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em;">Area</th>
                  ...
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="lg-cell">State</th>
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="xl-cell">Country</th>
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="xl-cell">Postal</th>
                  <th style="padding: 12px 16px; text-align: right; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="xl-cell">Radius</th>
                  <th style="padding: 12px 16px; text-align: center; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; width: 90px;">Service</th>
              ...
              <tbody>
                <tr *ngFor="let row of areas" (click)="viewArea(row)"
                  ...
                  <td style="padding: 14px 16px;" (click)="$event.stopPropagation()">
                    <input type="checkbox" (change)="toggleRow(row)" [checked]="selectedAreas.has(row)"
                      style="width: 16px; height: 16px; cursor: pointer; accent-color: #059669; border-radius: 4px;" />
                  </td>
                  <td style="padding: 14px 16px;">
                    <div>
                      <p style="font-weight: 600; color: #166534; margin: 0; line-height: 1.3;">{{ row.name }}</p>
                      <p *ngIf="row.area_code" style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0; font-family: monospace;">{{ row.area_code }}</p>
                    </div>
                  </td>
                  <td style="padding: 14px 16px; color: #6b7280;">{{ row.city?.name || '-' }}</td>
                  <td style="padding: 14px 16px; color: #6b7280; display: none;" class="lg-cell">{{ row.state?.name || '-' }}</td>
                  <td style="padding: 14px 16px; color: #6b7280; display: none;" class="xl-cell">{{ row.country?.name || '-' }}</td>
                  <td style="padding: 14px 16px; color: #6b7280; font-family: monospace; display: none;" class="xl-cell">{{ row.postal_zone || '-' }}</td>
                  <td style="padding: 14px 16px; text-align: right; color: #6b7280; font-family: monospace; display: none;" class="xl-cell">{{ row.delivery_radius ? (row.delivery_radius + ' km') : '-' }}</td>
                  <td style="padding: 14px 16px; text-align: center;">
                    <span class="material-icons" *ngIf="row.is_serviceable" style="font-size: 18px; color: #059669;">check_circle</span>
                    <span class="material-icons" *ngIf="!row.is_serviceable" style="font-size: 18px; color: #dc2626;">block</span>
                  </td>
                  <td style="padding: 14px 16px; text-align: center;">
                  ...
                  <td style="padding: 14px 16px; text-align: center; position: relative;" (click)="$event.stopPropagation()">
                    ...
                    <div *ngIf="openMenuId === row.id"
                      ...
                      <button *ngIf="row.is_serviceable" (click)="toggleService(row, false)"
                        style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; transition: background 0.1s ease; text-align: left;"
                        onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background=''">
                        <span class="material-icons" style="font-size: 18px; color: #dc2626;">local_shipping</span> Disable Service
                      </button>
                      <button *ngIf="!row.is_serviceable" (click)="toggleService(row, true)"
                        style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; transition: background 0.1s ease; text-align: left;"
                        onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background=''">
                        <span class="material-icons" style="font-size: 18px; color: #059669;">local_shipping</span> Enable Service
                      </button>
                      <div style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                      ...
                      <button *ngIf="!row.is_default" (click)="deleteArea(row)"
      ...
      <div *ngIf="importing" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 50; animation: fadeIn 0.2s;">
        <div style="background: white; border-radius: 20px; padding: 32px; text-align: center; box-shadow: 0 24px 48px rgba(0,0,0,0.2); max-width: 320px; width: calc(100% - 32px);">
          ...
          <p style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">Importing areas...</p>
        ...
        <div (click)="$event.stopPropagation()" style="background: white; border-radius: 20px; padding: 24px; max-width: 480px; width: calc(100% - 32px); box-shadow: 0 24px 48px rgba(0,0,0,0.2);">
          ...
          <div style="margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; margin-bottom: 8px;">
              ...
              <span style="color: #374151;">{{ importResult.success_count }} areas imported successfully</span>
            </div>
...
export class AreasComponent implements OnInit {
  ...
  loading = false;
  importing = false;
  importResult: AreaImportResult | null = null;
  ...
  loadAreas(): void {
    ...
    if (this.stateFilter) params['state_id'] = this.stateFilter.toString();
    ...
    this.areaApi.getAll(params).subscribe({
      next: (res) => {
        this.areas = res.data || [];
        this.totalCount = this.areas.length;
        this.totalPages = Math.ceil(this.totalCount / this.perPage) || 1;
        this.activeCount = this.areas.filter(a => a.status === 'active').length;
        this.serviceableCount = this.areas.filter(a => a.is_serviceable).length;
        this.defaultCount = this.areas.filter(a => a.is_default).length;
        this.updateStatCards();
        this.loading = false;
      },
      error: () => {
        this.loading = false;
        this.notification.error('Failed to load areas');
      },
    });
  }
  ...
  updateStatCards(): void {
    this.statCards = [
      { label: 'Total Areas', value: this.totalCount, icon: 'place', bg: '#d1fae5', color: '#047857' },
      ...
      { label: 'Non-Serviceable', value: this.totalCount - this.serviceableCount, icon: 'block', bg: '#fee2e2', color: '#dc2626' },
    ];
  }

  onSearchDebounce(): void {
    clearTimeout(this.searchTimeout);
    this.searchTimeout = setTimeout(() => { this.currentPage = 1; this.loadAreas(); }, 400);
  }

  clearSearch(): void { this.search = ''; this.currentPage = 1; this.loadAreas(); }
  onFilterChange(): void { this.currentPage = 1; this.loadAreas(); }
  clearFilters(): void {
    this.search = '';
  ...
  isAllSelected(): boolean {
    return this.areas.length > 0 && this.selectedAreas.size === this.areas.length;
  }

  toggleAllRows(event: any): void {
    if (event.target.checked) {
      this.areas.forEach(a => this.selectedAreas.add(a));
    } else {
  ...
  toggleRow(row: Area): void {
    if (this.selectedAreas.has(row)) {
      this.selectedAreas.delete(row);
    } else {
      this.selectedAreas.add(row);
    }
    this.openMenuId = null;
  }

  clearSelection(): void { this.selectedAreas.clear(); }

  toggleMenu(row: Area): void {
  ...
  viewArea(row: Area): void { this.router.navigate(['/admin/areas', row.uuid]); }
  editArea(row: Area): void { this.router.navigate(['/admin/areas', row.uuid, 'edit']); }

  setDefault(row: Area): void {
    this.openMenuId = null;
    if (window.confirm(`Set "${row.name}" as the default area?`)) {
      this.areaApi.setDefault(row.uuid).subscribe({
        next: () => { this.notification.success('Default area updated'); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  toggleService(row: Area, isServiceable: boolean): void {
    this.openMenuId = null;
    const label = isServiceable ? 'Enable service' : 'Disable service';
    if (window.confirm(`${label} for "${row.name}"?`)) {
      this.areaApi.setService(row.uuid, isServiceable).subscribe({
        next: () => { this.notification.success(`Service ${isServiceable ? 'enabled' : 'disabled'} for area`); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  setStatus(row: Area, status: string): void {
    this.openMenuId = null;
    const label = status === 'active' ? 'Activate' : 'Deactivate';
    if (window.confirm(`${label} "${row.name}"?`)) {
      this.areaApi.setStatus(row.uuid, status).subscribe({
        next: () => { this.notification.success(`Area ${status === 'active' ? 'activated' : 'deactivated'}`); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  deleteArea(row: Area): void {
    this.openMenuId = null;
    if (window.confirm(`Delete "${row.name}"? This action cannot be undone.`)) {
      this.areaApi.delete(row.uuid).subscribe({
        next: () => { this.notification.success('Area deleted'); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetActive(): void {
    const ids = [...this.selectedAreas].map(r => r.id);
    if (window.confirm(`Activate ${ids.length} selected areas?`)) {
      this.areaApi.bulkStatus(ids, 'active').subscribe({
        next: () => { this.notification.success('Areas activated'); this.clearSelection(); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetInactive(): void {
    const ids = [...this.selectedAreas].map(r => r.id);
    if (window.confirm(`Deactivate ${ids.length} selected areas?`)) {
      this.areaApi.bulkStatus(ids, 'inactive').subscribe({
        next: () => { this.notification.success('Areas deactivated'); this.clearSelection(); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  bulkEnableService(): void {
    const uuids = [...this.selectedAreas].map(r => r.uuid);
    if (window.confirm(`Enable service for ${uuids.length} selected areas?`)) {
      forkJoin(uuids.map(uuid => this.areaApi.setService(uuid, true))).subscribe({
        next: () => { this.notification.success('Service enabled for selected areas'); this.clearSelection(); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  bulkDisableService(): void {
    const uuids = [...this.selectedAreas].map(r => r.uuid);
    if (window.confirm(`Disable service for ${uuids.length} selected areas?`)) {
      forkJoin(uuids.map(uuid => this.areaApi.setService(uuid, false))).subscribe({
        next: () => { this.notification.success('Service disabled for selected areas'); this.clearSelection(); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkDelete(): void {
    const ids = [...this.selectedAreas].map(r => r.id);
    if (window.confirm(`Delete ${ids.length} selected areas? This cannot be undone.`)) {
      this.areaApi.bulkDelete(ids).subscribe({
        next: () => { this.notification.success('Areas deleted'); this.clearSelection(); this.loadAreas(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  onFileSelected(event: Event): void {
    ...
    this.importing = true;
    this.areaApi.import(file).subscribe({
      next: (res) => { this.importing = false; this.importResult = res.data || null; this.loadAreas(); input.value = ''; },
  ...
  exportAreas(): void {
    ...
    if (this.stateFilter) filters['state_id'] = this.stateFilter.toString();
    ...
    this.areaApi.export(filters).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'areas-export.csv'; a.click();

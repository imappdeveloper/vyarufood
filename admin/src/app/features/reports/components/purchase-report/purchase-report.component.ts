  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading purchase report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Purchase Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Purchase Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Supplier orders, quantities, and spending overview</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Purchase Orders</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Supplier Name</th>
            ...
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px; font-weight: 600; color: #374151;">{{ row.supplier_name ?? '—' }}</td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; align-items: center; gap: 4px;"
                    [style.background]="row.order_status === 'completed' ? '#d1fae5' : row.order_status === 'pending' || row.order_status === 'processing' ? '#fef3c7' : row.order_status === 'cancelled' ? '#fee2e2' : row.order_status === 'delivered' ? '#dbeafe' : '#f3f4f6'"
                    [style.color]="row.order_status === 'completed' ? '#047857' : row.order_status === 'pending' || row.order_status === 'processing' ? '#b45309' : row.order_status === 'cancelled' ? '#dc2626' : row.order_status === 'delivered' ? '#1d4ed8' : '#6b7280'">
                    {{ row.order_status ?? '—' }}
                  </span>
                </td>
                <td style="padding: 12px 12px; text-align: right; color: #374151;">{{ row.order_count?.toLocaleString('en-IN') ?? '—' }}</td>
        ...
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No purchase data found</h3>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Distribution of purchase order statuses"
        chartType="donut"
...
export class PurchaseReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'supplier_name', 'order_status', 'order_count', 'total_amount'];
  ...
  get statCards() {
    ...
    for (const r of rows) {
      if (r.order_status === 'completed' || r.order_status === 'delivered') completed += r.order_count ?? 0;
      if (r.order_status === 'pending' || r.order_status === 'processing') pending += r.order_count ?? 0;
    }
    return [
      ...
      { label: 'Completed', value: completed.toLocaleString('en-IN'), icon: 'check_circle', bg: '#d1fae5', color: '#047857' },
      { label: 'Pending', value: pending.toLocaleString('en-IN'), icon: 'schedule', bg: '#fef3c7', color: '#d97706' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('purchases', {
  ...
  private buildChartData(): void {
    ...
    this.chartSeries = { series: [{ name: 'Orders', data: this.chartValues }] };
  }
  ...
  exportReport(): void {
    ...
    a.href = url; a.download = 'purchase-report.csv'; a.click();

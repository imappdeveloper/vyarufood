  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading sales report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Sales Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Sales Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Revenue and sales performance analytics</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Sales Data</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Orders</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Revenue</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Discount</th>
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Avg Order Value</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px; text-align: right; color: #374151;">{{ row.total_orders?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 12px; text-align: right; font-weight: 700; color: #059669;">{{ formatCurrency(row.total_revenue) }}</td>
                <td style="padding: 12px 12px; text-align: right; color: #dc2626;">{{ formatCurrency(row.total_discount) }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; color: #374151;">{{ formatCurrency(row.avg_order_value) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No sales data found</h3>
        ...
        <div *ngIf="reportData.summary" style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap; padding: 12px 16px; border-top: 1px solid #f3f4f6; background: #f9fafb;">
          <span style="font-size: 12px; color: #6b7280;">Total Orders: <strong style="color: #374151;">{{ reportData.summary.total_orders?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Total Revenue: <strong style="color: #059669;">{{ formatCurrency(reportData.summary.total_revenue) }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Total Discount: <strong style="color: #dc2626;">{{ formatCurrency(reportData.summary.total_discount) }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Avg Order Value: <strong style="color: #374151;">{{ formatCurrency(reportData.summary.avg_order_value) }}</strong></span>
        </div>
      </div>
...
export class SalesReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'total_orders', 'total_revenue', 'total_discount', 'avg_order_value'];
  ...
  get statCards() {
    ...
    return [
      { label: 'Total Orders', value: (s?.total_orders ?? 0).toLocaleString('en-IN'), icon: 'shopping_cart', bg: '#d1fae5', color: '#059669' },
      { label: 'Total Revenue', value: this.formatCurrency(s?.total_revenue), icon: 'payments', bg: '#d1fae5', color: '#047857' },
      { label: 'Total Discount', value: this.formatCurrency(s?.total_discount), icon: 'percent', bg: '#fef3c7', color: '#d97706' },
      { label: 'Avg Order Value', value: this.formatCurrency(s?.avg_order_value), icon: 'trending_up', bg: '#dbeafe', color: '#2563eb' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('sales', {
  ...
  private buildChartData(): void {
    ...
    this.chartSeries = { series: [{ name: 'Revenue', data: this.chartValues }] };
  }

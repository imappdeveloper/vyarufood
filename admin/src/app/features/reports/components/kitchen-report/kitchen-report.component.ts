  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading kitchen report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Kitchen Report</span>
      </div>

      <div style="margin-bottom: 24px;">
        <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Kitchen Report</h1>
        <p style="font-size: 13px; color: #9ca3af; margin: 0;">Kitchen production and batch status overview</p>
      </div>
      ...
      <div *ngIf="reportData?.data?.length" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Kitchen Production Data</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Kitchen Name</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Production Status</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Batch Count</th>
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Produced</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px; font-weight: 600; color: #374151;">{{ row.kitchen_name ?? '—' }}</td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600;"
                    [style.background]="statusColor(row.production_status).bg" [style.color]="statusColor(row.production_status).fg">
                    {{ row.production_status?.replace('_', ' ') | titlecase }}
                  </span>
                </td>
                <td style="padding: 12px 12px; text-align: right; font-weight: 600; color: #374151;">{{ row.batch_count?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 700; color: #059669;">{{ row.total_produced?.toLocaleString('en-IN') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="reportData.summary" style="padding: 12px 16px; background: #f9fafb; border-top: 1px solid #e5e7eb; display: flex; flex-wrap: wrap; gap: 24px; font-size: 13px;">
          <span *ngFor="let entry of summaryEntries" style="color: #6b7280;">
            {{ entry.status | titlecase }}: <strong style="color: #166534;">{{ entry.count?.toLocaleString('en-IN') }} batches</strong>
          </span>
        </div>
      </div>
      ...
      <div *ngIf="reportData && !reportData?.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
        ...
        <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No kitchen data found</h3>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Distribution of production batches across statuses"
        chartType="donut"
...
export class KitchenReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'kitchen_name', 'production_status', 'batch_count', 'total_produced'];
  ...
  statusColor(status: string): { bg: string; fg: string } {
    switch (status) {
      ...
      case 'pending': return { bg: '#fffbeb', fg: '#b45309' };
      case 'failed': return { bg: '#fef2f2', fg: '#b91c1c' };
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('kitchen', {
  ...
  private buildSummary(): void {
    ...
    for (const row of this.reportData.data) {
      const status = row.production_status ?? 'unknown';
      map.set(status, (map.get(status) ?? 0) + (row.batch_count ?? 0));
    }

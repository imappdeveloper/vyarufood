  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading notification report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Notification Report</span>
      </div>

      <div style="margin-bottom: 24px;">
        <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Notification Report</h1>
        <p style="font-size: 13px; color: #9ca3af; margin: 0;">Channel-wise notification delivery overview</p>
      </div>
      ...
      <div *ngIf="reportData?.data?.length" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Notification Details</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Delivery Status</th>
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Notification Count</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600;"
                    [style.background]="channelBadge(row.channel).bg" [style.color]="channelBadge(row.channel).fg">
                    {{ row.channel ?? '—' }}
                  </span>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600;"
                    [style.background]="statusBadge(row.delivery_status).bg" [style.color]="statusBadge(row.delivery_status).fg">
                    {{ row.delivery_status ?? '—' }}
                  </span>
                </td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 600; color: #374151;">{{ row.notification_count?.toLocaleString('en-IN') ?? '—' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="reportData.summary" style="padding: 12px 16px; background: #f9fafb; border-top: 1px solid #e5e7eb; display: flex; flex-wrap: wrap; gap: 24px; font-size: 13px;">
          <span style="color: #6b7280;">Total Notifications: <strong style="color: #166534;">{{ reportData.summary.total_notifications?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="color: #6b7280;">Delivered: <strong style="color: #059669;">{{ reportData.summary.total_delivered?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="color: #6b7280;">Failed: <strong style="color: #dc2626;">{{ reportData.summary.total_failed?.toLocaleString('en-IN') ?? '—' }}</strong></span>
        </div>
      </div>
      ...
      <div *ngIf="reportData && !reportData?.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
        ...
        <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No notification data found</h3>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Channel-wise distribution"
        chartType="donut"
...
export class NotificationReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'channel', 'delivery_status', 'notification_count'];
  ...
  channelBadge(channel: string): { bg: string; fg: string } {
    switch (channel) {
      case 'sms': return { bg: '#eff6ff', fg: '#1d4ed8' };
      case 'email': return { bg: '#ecfdf5', fg: '#047857' };
      case 'push': return { bg: '#faf5ff', fg: '#7c3aed' };
      case 'whatsapp': return { bg: '#fffbeb', fg: '#b45309' };
      default: return { bg: '#f9fafb', fg: '#374151' };
    }
  }

  statusBadge(status: string): { bg: string; fg: string } {
    if (status === 'delivered' || status === 'sent') return { bg: '#ecfdf5', fg: '#047857' };
    if (status === 'pending' || status === 'queued') return { bg: '#fffbeb', fg: '#b45309' };
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('notifications', {
  ...
  private buildChartData(): void {
    ...
    this.chartSeries = { series: [{ name: 'Count', data: this.chartValues }] };
  }
}

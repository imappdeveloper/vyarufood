  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading supplier report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Supplier Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Supplier Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Supplier-wise purchase overview</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Supplier Details</h3>
        </div>
        <div *ngIf="reportData.data?.length" style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                <th style="padding: 10px 12px 10px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Supplier Name</th>
                ...
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Purchased</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td [attr.data-idx]="i" style="padding: 12px 12px 12px 16px; font-weight: 600; color: #374151;">{{ row.supplier_name ?? '—' }}</td>
                <td style="padding: 12px 12px; text-align: right; color: #374151;">{{ row.order_count?.toLocaleString('en-IN') ?? '—' }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 700; color: #059669;">{{ formatCurrency(row.total_purchased) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No supplier data found</h3>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Supplier purchase distribution"
        chartType="bar"
...
export class SupplierReportComponent implements OnInit {
  ...
  displayedColumns = ['supplier_name', 'order_count', 'total_purchased'];
  ...
  get statCards() {
    ...
    return [
      { label: 'Total Suppliers', value: (s?.total_suppliers ?? 0).toLocaleString('en-IN'), icon: 'store', bg: '#dbeafe', color: '#2563eb' },
      { label: 'Total Orders', value: (s?.total_orders ?? 0).toLocaleString('en-IN'), icon: 'shopping_cart', bg: '#fef3c7', color: '#d97706' },
      { label: 'Total Purchased', value: this.formatCurrency(s?.total_purchased), icon: 'payments', bg: '#d1fae5', color: '#059669' },
      { label: 'Suppliers Listed', value: rows.length.toLocaleString('en-IN'), icon: 'storefront', bg: '#d1fae5', color: '#047857' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('suppliers', {
  ...
  exportReport(): void {
    ...
    a.href = url; a.download = 'supplier-report.csv'; a.click();

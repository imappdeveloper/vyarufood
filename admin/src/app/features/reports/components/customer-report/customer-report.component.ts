  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading customer report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Customer Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Customer Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Customer acquisition and activity overview</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Customer Data</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Customers</th>
                <th style="padding: 10px 12px; text-align: center; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Active</th>
                <th style="padding: 10px 16px 10px 12px; text-align: center; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Inactive</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px; text-align: right; font-weight: 700; color: #374151;">{{ row.total_customers?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 12px; text-align: center;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; background: #d1fae5; color: #047857;">{{ row.active?.toLocaleString('en-IN') }}</span>
                </td>
                <td style="padding: 12px 16px 12px 12px; text-align: center;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; background: #fee2e2; color: #dc2626;">{{ row.inactive?.toLocaleString('en-IN') }}</span>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No customer data found</h3>
        ...
        <div *ngIf="reportData.summary" style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap; padding: 12px 16px; border-top: 1px solid #f3f4f6; background: #f9fafb;">
          <span style="font-size: 12px; color: #6b7280;">Total Customers: <strong style="color: #374151;">{{ reportData.summary.total_customers?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Active: <strong style="color: #059669;">{{ reportData.summary.active?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Inactive: <strong style="color: #dc2626;">{{ reportData.summary.inactive?.toLocaleString('en-IN') ?? '—' }}</strong></span>
        </div>
      </div>
      ...
      <div *ngIf="topCustomers?.length" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; gap: 8px; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <span class="material-icons" style="font-size: 18px; color: #d97706;">emoji_events</span>
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Top Customers</h3>
        </div>
        <div style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                <th style="padding: 10px 12px 10px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Name</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Email</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Order Count</th>
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Total Spent</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of topCustomers; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                [style.background]="i % 2 === 0 ? 'transparent' : '#f9fafb'"
                onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background=(this.getAttribute('data-idx') % 2 === 0) ? 'transparent' : '#f9fafb'">
                <td [attr.data-idx]="i" style="padding: 12px 12px 12px 16px; font-weight: 700; color: #374151;">{{ row.name }}</td>
                <td style="padding: 12px 12px; color: #9ca3af;">{{ row.email }}</td>
                <td style="padding: 12px 12px; text-align: right; font-weight: 700; color: #374151;">{{ row.order_count?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 700; color: #059669;">{{ formatCurrency(row.total_spent) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class CustomerReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'total_customers', 'active', 'inactive'];
  topColumns = ['name', 'email', 'order_count', 'total_spent'];
  ...
  get statCards() {
    ...
    return [
      { label: 'Total Customers', value: (s?.total_customers ?? 0).toLocaleString('en-IN'), icon: 'people', bg: '#d1fae5', color: '#059669' },
      { label: 'Active', value: (s?.active ?? 0).toLocaleString('en-IN'), icon: 'check_circle', bg: '#d1fae5', color: '#047857' },
      { label: 'Inactive', value: (s?.inactive ?? 0).toLocaleString('en-IN'), icon: 'cancel', bg: '#fee2e2', color: '#dc2626' },
      { label: 'Periods', value: rows.length.toLocaleString('en-IN'), icon: 'calendar_month', bg: '#fef3c7', color: '#d97706' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('customers', {
      ...
      next: (res) => {
        ...
        this.topCustomers = this.reportData?.chart?.top_customers ?? [];
        this.loading = false;
      },
  ...
  exportReport(): void {
    ...
    a.href = url; a.download = 'customer-report.csv'; a.click();

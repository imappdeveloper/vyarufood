  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading subscription report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Subscription Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Subscription Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Subscription status and plan distribution</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Subscription Data</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Subscription Status</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Plan Name</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Count</th>
                <th style="padding: 10px 16px 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Amount</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; align-items: center; gap: 4px;"
                    [style.background]="row.subscription_status === 'active' ? '#d1fae5' : row.subscription_status === 'paused' ? '#fef3c7' : row.subscription_status === 'cancelled' ? '#fee2e2' : row.subscription_status === 'pending' ? '#dbeafe' : '#f3f4f6'"
                    [style.color]="row.subscription_status === 'active' ? '#047857' : row.subscription_status === 'paused' ? '#b45309' : row.subscription_status === 'cancelled' ? '#dc2626' : row.subscription_status === 'pending' ? '#1d4ed8' : '#6b7280'">
                    {{ row.subscription_status?.replace('_', ' ') | titlecase }}
                  </span>
                </td>
                <td style="padding: 12px 12px; font-weight: 600; color: #374151;">{{ row.plan_name ?? '—' }}</td>
                <td style="padding: 12px 12px; text-align: right; font-weight: 700; color: #374151;">{{ row.count?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 700; color: #059669;">{{ formatCurrency(row.amount) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No subscription data found</h3>
        ...
        <div *ngIf="reportData.summary" style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap; padding: 12px 16px; border-top: 1px solid #f3f4f6; background: #f9fafb;">
          <span *ngFor="let entry of summaryEntries" style="font-size: 12px; color: #6b7280;">
            {{ entry.status | titlecase }}: <strong style="color: #374151;">{{ entry.count?.toLocaleString('en-IN') }} subscriptions</strong>
          </span>
        </div>
      </div>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Distribution of subscriptions across statuses"
        chartType="donut"
...
export class SubscriptionReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'subscription_status', 'plan_name', 'count', 'amount'];

  chartLabels: string[] = [];
  chartValues: number[] = [];
  chartSeries: any = null;

  summaryEntries: { status: string; count: number }[] = [];

  get statCards() {
    const rows = this.reportData?.data ?? [];
    let total = 0, amount = 0;
    for (const r of rows) { total += r.count ?? 0; amount += r.amount ?? 0; }
    const map = new Map<string, number>();
    for (const s of this.summaryEntries) map.set(s.status, s.count);
    return [
      { label: 'Total Subscriptions', value: total.toLocaleString('en-IN'), icon: 'subscriptions', bg: '#d1fae5', color: '#059669' },
      { label: 'Active', value: (map.get('active') ?? 0).toLocaleString('en-IN'), icon: 'check_circle', bg: '#d1fae5', color: '#047857' },
      { label: 'Paused', value: (map.get('paused') ?? 0).toLocaleString('en-IN'), icon: 'pause_circle', bg: '#fef3c7', color: '#d97706' },
      { label: 'Cancelled', value: (map.get('cancelled') ?? 0).toLocaleString('en-IN'), icon: 'cancel', bg: '#fee2e2', color: '#dc2626' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('subscriptions', {
  ...
  private buildChartData(): void {
    ...
    if (typeof chart === 'object' && !Array.isArray(chart)) {
      this.chartLabels = Object.keys(chart).map(k => k.replace('_', ' '));
      this.chartValues = Object.values(chart).map(Number);
    }
    this.chartSeries = { series: this.chartValues };
  }

  private buildSummary(): void {
    if (!this.reportData?.data?.length) { this.summaryEntries = []; return; }
    const map = new Map<string, number>();
    for (const row of this.reportData.data) {
      const status = row.subscription_status ?? 'unknown';
      map.set(status, (map.get(status) ?? 0) + (row.count ?? 0));
    }
    this.summaryEntries = Array.from(map.entries()).map(([status, count]) => ({ status, count }));
  }
  ...
  exportReport(): void {
    ...
    a.href = url; a.download = 'subscription-report.csv'; a.click();

  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading expense report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Expense Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Expense Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Category-wise expense breakdown</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Expense Details</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Expense Status</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Expense Count</th>
            ...
            <tbody>
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px; font-weight: 600; color: #374151;">{{ row.category ?? '—' }}</td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; align-items: center; gap: 4px;"
                    [style.background]="row.expense_status === 'approved' ? '#d1fae5' : row.expense_status === 'pending' ? '#fef3c7' : row.expense_status === 'rejected' ? '#fee2e2' : row.expense_status === 'paid' ? '#dbeafe' : '#f3f4f6'"
                    [style.color]="row.expense_status === 'approved' ? '#047857' : row.expense_status === 'pending' ? '#b45309' : row.expense_status === 'rejected' ? '#dc2626' : row.expense_status === 'paid' ? '#1d4ed8' : '#6b7280'">
                    {{ row.expense_status ?? '—' }}
                  </span>
                </td>
                <td style="padding: 12px 12px; text-align: right; color: #374151;">{{ row.expense_count?.toLocaleString('en-IN') ?? '—' }}</td>
                <td style="padding: 12px 16px 12px 12px; text-align: right; font-weight: 700; color: #dc2626;">{{ formatCurrency(row.total_amount) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No expense data found</h3>
        ...
        <div *ngIf="reportData.summary" style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap; padding: 12px 16px; border-top: 1px solid #f3f4f6; background: #f9fafb;">
          <span style="font-size: 12px; color: #6b7280;">Total Expenses: <strong style="color: #374151;">{{ reportData.summary.total_expenses?.toLocaleString('en-IN') ?? '—' }}</strong></span>
          <span style="font-size: 12px; color: #6b7280;">Total Amount: <strong style="color: #dc2626;">{{ formatCurrency(reportData.summary.total_amount) }}</strong></span>
        </div>
      </div>
...
export class ExpenseReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'category', 'expense_status', 'expense_count', 'total_amount'];
  ...
  get statCards() {
    ...
    for (const r of rows) {
      if (r.expense_status === 'approved' || r.expense_status === 'paid') approved += r.expense_count ?? 0;
      if (r.expense_status === 'pending') pending += r.expense_count ?? 0;
    }
    return [
      { label: 'Total Expenses', value: (s?.total_expenses ?? 0).toLocaleString('en-IN'), icon: 'receipt_long', bg: '#d1fae5', color: '#059669' },
      { label: 'Total Amount', value: this.formatCurrency(s?.total_amount), icon: 'payments', bg: '#fef3c7', color: '#d97706' },
      { label: 'Approved', value: approved.toLocaleString('en-IN'), icon: 'check_circle', bg: '#d1fae5', color: '#047857' },
      { label: 'Pending', value: pending.toLocaleString('en-IN'), icon: 'schedule', bg: '#dbeafe', color: '#2563eb' },
    ];
  }
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('expenses', {

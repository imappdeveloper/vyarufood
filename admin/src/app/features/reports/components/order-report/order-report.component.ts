  ...
  template: `
    <div *ngIf="loading && !reportData" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading order report...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || reportData" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Order Report</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Order Report</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Order volume and status distribution</p>
        </div>
      ...
      <div *ngIf="reportData" style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 16px;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #f3f4f6;">
          <h3 style="font-size: 13px; font-weight: 700; color: #374151; margin: 0;">Order Data</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Order Status</th>
                ...
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Item Count</th>
              ...
              <tr *ngFor="let row of reportData.data; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; align-items: center; gap: 4px;"
                    [style.background]="row.order_status === 'delivered' ? '#d1fae5' : row.order_status === 'confirmed' ? '#dbeafe' : row.order_status === 'pending' ? '#fef3c7' : row.order_status === 'cancelled' ? '#fee2e2' : row.order_status === 'out_for_delivery' ? '#ede9fe' : '#f3f4f6'"
                    [style.color]="row.order_status === 'delivered' ? '#047857' : row.order_status === 'confirmed' ? '#1d4ed8' : row.order_status === 'pending' ? '#b45309' : row.order_status === 'cancelled' ? '#dc2626' : row.order_status === 'out_for_delivery' ? '#7c3aed' : '#6b7280'">
                    {{ row.order_status?.replace('_', ' ') | titlecase }}
                  </span>
                </td>
                <td style="padding: 12px 12px; text-align: right; font-weight: 700; color: #374151;">{{ row.order_count?.toLocaleString('en-IN') }}</td>
                <td style="padding: 12px 12px; text-align: right; color: #374151;">{{ row.item_count?.toLocaleString('en-IN') }}</td>
        ...
        <div *ngIf="!reportData.data?.length" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No order data found</h3>
        ...
        <div *ngIf="reportData.summary" style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap; padding: 12px 16px; border-top: 1px solid #f3f4f6; background: #f9fafb;">
          <span *ngFor="let entry of summaryEntries" style="font-size: 12px; color: #6b7280;">
            {{ entry.status | titlecase }}: <strong style="color: #374151;">{{ entry.count?.toLocaleString('en-IN') }} orders</strong>
          </span>
        </div>
      </div>
      ...
      <app-chart-card
        *ngIf="chartLabels.length && chartValues.length"
        ...
        subtitle="Distribution of orders across statuses"
        chartType="donut"
...
export class OrderReportComponent implements OnInit {
  ...
  displayedColumns = ['period', 'order_status', 'order_count', 'item_count', 'total_amount'];
  ...
  get statCards() {
    ...
    let orders = 0, items = 0, amount = 0;
    for (const r of rows) { orders += r.order_count ?? 0; items += r.item_count ?? 0; amount += r.total_amount ?? 0; }
    return [
      { label: 'Total Orders', value: orders.toLocaleString('en-IN'), icon: 'shopping_cart', bg: '#d1fae5', color: '#059669' },
      { label: 'Total Items', value: items.toLocaleString('en-IN'), icon: 'local_mall', bg: '#dbeafe', color: '#2563eb' },
      { label: 'Total Amount', value: this.formatCurrency(amount), icon: 'payments', bg: '#d1fae5', color: '#047857' },
  ...
  loadReport(): void {
    this.loading = true;
    this.reportApi.getReport('orders', {
  ...
  private buildSummary(): void {
    ...
    for (const row of this.reportData.data) {
      ...
      map.set(status, (map.get(status) ?? 0) + (row.order_count ?? 0));
    }
  ...
  exportReport(): void {
    ...
    a.href = url; a.download = 'order-report.csv'; a.click();

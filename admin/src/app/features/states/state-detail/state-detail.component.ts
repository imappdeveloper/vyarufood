  ...
  template: `
    <div *ngIf="loading" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading state details...</p>
      </div>
    </div>

    <div *ngIf="!loading && state" style="animation: fadeIn 0.3s ease-out;">
        ...
        <div style="max-width: 1200px; margin: 0 auto; position: relative; z-index: 2;">
          ...
          <div style="display: flex; flex-direction: column; gap: 12px;">
            <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
              <h1 style="font-size: 28px; font-weight: 800; color: white; margin: 0;">{{ state.name }}</h1>
              <span *ngIf="state.state_code" style="padding: 4px 12px; background: rgba(255,255,255,0.2); border-radius: 8px; font-size: 11px; font-weight: 700; font-family: monospace; color: white; letter-spacing: 1px; text-transform: uppercase;">{{ state.state_code }}</span>
            </div>
            <p *ngIf="state.country" style="font-size: 14px; color: rgba(255,255,255,0.85); margin: 0;">{{ state.country.name }}</p>
          </div>
          <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-top: 20px;">
            <button (click)="setDefault()"
              ...
              [style.background]="state.is_default ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.15)'"
              onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="if(!state.is_default)this.style.background='rgba(255,255,255,0.15)'">
              <span class="material-icons" style="font-size: 18px;" [style.color]="state.is_default ? '#fbbf24' : ''">{{ state.is_default ? 'star' : 'star_outline' }}</span>
              {{ state.is_default ? 'Default' : 'Set Default' }}
            </button>
            <button (click)="toggleStatus()"
              style="padding: 8px 16px; background: rgba(255,255,255,0.15); color: white; font-weight: 600; border-radius: 10px; border: 1px solid rgba(255,255,255,0.2); font-size: 13px; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"
              onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="this.style.background='rgba(255,255,255,0.15)'">
              <span class="material-icons" style="font-size: 18px;">{{ state.status === 'active' ? 'block' : 'check_circle' }}</span>
              {{ state.status === 'active' ? 'Deactivate' : 'Activate' }}
            </button>
            <a [routerLink]="['/admin/states', state.uuid, 'edit']"
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
              <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 24px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
                  <div>
                    <p style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af; margin: 0 0 4px 0;">State Name</p>
                    <p style="font-size: 14px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.name }}</p>
                  </div>
                  <div>
                    ...
                    <p style="font-size: 14px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.country?.name || '-' }}</p>
                  </div>
                  <div>
                    <p style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af; margin: 0 0 4px 0;">State Code</p>
                    <span *ngIf="state.state_code" style="display: inline-block; padding: 3px 10px; background: #d1fae5; border-radius: 6px; font-size: 12px; font-weight: 700; font-family: monospace; color: #047857;">{{ state.state_code }}</span>
                    <span *ngIf="!state.state_code" style="font-size: 14px; color: #9ca3af;">-</span>
                  </div>
                  <div>
                    <p style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af; margin: 0 0 4px 0;">Abbreviation</p>
                    <p style="font-size: 14px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.abbreviation || '-' }}</p>
                  </div>
                  <div>
                    <p style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af; margin: 0 0 4px 0;">GST Code</p>
                    <p style="font-size: 14px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.gst_code || '-' }}</p>
                  </div>
                </div>
              </div>
              ...
              <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 24px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                  <div>
                    ...
                    <p style="font-size: 14px; font-weight: 600; font-family: monospace; color: #1f2937; margin: 0;">{{ state.latitude || '-' }}</p>
                  </div>
                  <div>
                    ...
                    <p style="font-size: 14px; font-weight: 600; font-family: monospace; color: #1f2937; margin: 0;">{{ state.longitude || '-' }}</p>
                  </div>
                </div>
              </div>

              <div *ngIf="state.remarks" style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 24px;">
                ...
                <p style="font-size: 14px; color: #4b5563; line-height: 1.6; margin: 0;">{{ state.remarks }}</p>
              </div>
            </div>
                ...
                <div>
                  <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f3f4f6;">
                    ...
                    <span style="display: inline-flex; align-items: center; gap: 4px; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                      [style.background]="getStatusBg(state.status)" [style.color]="getStatusColor(state.status)">
                      <span *ngIf="state.status === 'active'" style="width: 6px; height: 6px; border-radius: 50%; background: currentColor;"></span>
                      {{ state.status | titlecase }}
                    </span>
                  </div>
                  <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f3f4f6;">
                    <span style="font-size: 13px; color: #6b7280;">Default</span>
                    <span *ngIf="state.is_default" style="display: inline-flex; align-items: center; gap: 4px; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; background: #fef3c7; color: #d97706;">
                      <span class="material-icons" style="font-size: 12px;">star</span> Yes
                    </span>
                    <span *ngIf="!state.is_default" style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; background: #f3f4f6; color: #6b7280;">No</span>
                  </div>
                  <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 0;">
                    <span style="font-size: 13px; color: #6b7280;">Sort Order</span>
                    <span style="font-size: 13px; font-weight: 700; color: #166534;">{{ state.sort_order }}</span>
                  </div>
                </div>
              </div>
              ...
              <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 24px;">
                ...
                <div>
                  <div style="padding: 10px 0; border-bottom: 1px solid #f3f4f6;">
                    ...
                    <p style="font-size: 13px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.created_at | date:'MMM d, y, h:mm a' }}</p>
                  </div>
                  <div style="padding: 10px 0;">
                    <p style="font-size: 11px; color: #9ca3af; margin: 0 0 4px 0;">Updated At</p>
                    <p style="font-size: 13px; font-weight: 600; color: #1f2937; margin: 0;">{{ state.updated_at | date:'MMM d, y, h:mm a' }}</p>
                  </div>
                </div>
              </div>
              ...
              <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 24px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px;">
                  <a [routerLink]="['/admin/states', state.uuid, 'edit']"
                    ...
                    <span class="material-icons" style="font-size: 18px; color: #047857;">edit</span> Edit State
                  </a>
                  <button *ngIf="!state.is_default" (click)="setDefault()"
                  ...
                  <button (click)="toggleStatus()"
                    style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 10px 14px; background: none; border: 1px solid #e5e7eb; border-radius: 10px; font-size: 13px; color: #374151; cursor: pointer; transition: all 0.15s ease; text-align: left; box-sizing: border-box;"
                    onmouseover="this.style.borderColor='#059669'; this.style.background='#f0fdf4'" onmouseout="this.style.borderColor='#e5e7eb'; this.style.background=''">
                    <span class="material-icons" style="font-size: 18px;" [style.color]="state.status === 'active' ? '#dc2626' : '#059669'">{{ state.status === 'active' ? 'block' : 'check_circle' }}</span>
                    {{ state.status === 'active' ? 'Deactivate' : 'Activate' }}
                  </button>
                  <div *ngIf="!state.is_default" style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                  <button *ngIf="!state.is_default" (click)="deleteState()"
                    ...
                    <span class="material-icons" style="font-size: 18px;">delete</span> Delete State
                  </button>
                </div>
              </div>
            </div>
...
export class StateDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (uuid) { this.loadState(uuid); } else { this.router.navigate(['/admin/states']); }
  }
  ...
  loadState(uuid: string): void {
    ...
    this.stateApi.getById(uuid).subscribe({
      next: (res) => {
        if (res.data) { this.state = res.data; } else { this.router.navigate(['/admin/states']); }
        this.loading = false;
      },
      error: () => { this.loading = false; this.notification.error('Failed to load state'); this.router.navigate(['/admin/states']); },
    });
  }
  ...
  setDefault(): void {
    ...
    if (window.confirm(`Set "${this.state.name}" as the default state?`)) {
      this.stateApi.setDefault(this.state.uuid).subscribe({
        next: () => { this.notification.success('Default state updated'); this.loadState(this.state!.uuid); },
  ...
  toggleStatus(): void {
    ...
    if (window.confirm(`${label} "${this.state.name}"?`)) {
      this.stateApi.setStatus(this.state.uuid, newStatus).subscribe({
        next: () => { this.notification.success(`State ${newStatus === 'active' ? 'activated' : 'deactivated'}`); this.loadState(this.state!.uuid); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  deleteState(): void {
    if (!this.state) return;
    if (window.confirm(`Delete "${this.state.name}"? This action cannot be undone.`)) {
      this.stateApi.delete(this.state.uuid).subscribe({
        next: () => { this.notification.success('State deleted'); this.router.navigate(['/admin/states']); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

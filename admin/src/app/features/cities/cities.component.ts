...
import { City, CityImportResult } from '../../core/models/master/city.model';
    ...
    <section style="position: relative; background: linear-gradient(135deg, #059669, #047857, #166534); padding: 48px 32px 80px; overflow: hidden;">
      ...
      <div style="max-width: 1200px; margin: 0 auto; position: relative; z-index: 2;">
        <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; color: rgba(255,255,255,0.75); margin-bottom: 16px;">
          ...
          <span style="color: white; font-weight: 500;">Cities</span>
        </div>
        <div style="display: flex; align-items: flex-start; justify-content: space-between;">
          <div>
            ...
            <p style="font-size: 14px; color: rgba(255,255,255,0.85); margin: 0;">Manage all cities and urban areas</p>
          </div>
          <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
            ...
            <button (click)="exportCities()"
              style="padding: 8px 16px; background: rgba(255,255,255,0.15); color: white; font-weight: 600; border-radius: 10px; border: 1px solid rgba(255,255,255,0.2); font-size: 13px; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"
              onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="this.style.background='rgba(255,255,255,0.15)'">
              <span class="material-icons" style="font-size: 18px;">download</span>
              Export
            </button>
            <a routerLink="/admin/cities/create"
    ...
    <section style="max-width: 1200px; margin: 24px auto 60px; padding: 0 24px;">
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; padding: 16px 20px; margin-bottom: 16px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 200px; position: relative;">
          ...
          <input [(ngModel)]="search" (keyup.enter)="loadCities()" (ngModelChange)="onSearchDebounce()" placeholder="Search cities..."
        ...
        <select [(ngModel)]="stateFilter" (change)="onFilterChange()"
          style="padding: 10px 14px; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 13px; color: #374151; outline: none; transition: all 0.2s ease; background: #f9fafb; cursor: pointer; min-width: 160px;"
          onfocus="this.style.borderColor='#059669'; this.style.boxShadow='0 0 0 3px rgba(5,150,105,0.1)'"
          onblur="this.style.borderColor='#e5e7eb'; this.style.boxShadow=''">
      ...
      <div *ngIf="selectedCities.size > 0" style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; margin-bottom: 16px; padding: 12px 20px; display: flex; align-items: center; gap: 12px; animation: fadeSlideUp 0.2s ease-out;">
        <span style="font-size: 13px; font-weight: 700; color: #047857;">{{ selectedCities.size }} selected</span>
      ...
      <div style="background: white; border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden;">
        <div *ngIf="loading" style="display: flex; align-items: center; justify-content: center; padding: 80px 20px;">
          <div style="text-align: center;">
            ...
            <p style="font-size: 13px; color: #9ca3af;">Loading cities...</p>
          </div>
        </div>

        <div *ngIf="!loading && cities.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 80px 20px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No cities found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 320px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || countryFilter || stateFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !countryFilter && !stateFilter">Get started by adding your first city</span>
          </p>
          <a *ngIf="!search && !statusFilter && !countryFilter && !stateFilter" routerLink="/admin/cities/create"
            ...
            <span class="material-icons" style="font-size: 18px;">add</span> Add City
          </a>
          <button *ngIf="search || statusFilter || countryFilter || stateFilter" (click)="clearFilters()"
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              <thead>
                <tr style="background: #f0fdf4; border-bottom: 2px solid #d1fae5;">
                  ...
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em;">State</th>
                  <th style="padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #047857; text-transform: uppercase; letter-spacing: 0.05em; display: none;" class="lg-cell">Country</th>
              ...
              <tbody>
                <tr *ngFor="let row of cities" (click)="viewCity(row)"
                  ...
                  <td style="padding: 14px 16px;" (click)="$event.stopPropagation()">
                    <input type="checkbox" (change)="toggleRow(row)" [checked]="selectedCities.has(row)"
                      style="width: 16px; height: 16px; cursor: pointer; accent-color: #059669; border-radius: 4px;" />
                  </td>
                  <td style="padding: 14px 16px;">
                    <div>
                      <div style="display: flex; align-items: center; gap: 6px;">
                        <span style="font-weight: 600; color: #166534; line-height: 1.3;">{{ row.name }}</span>
                        <span *ngIf="row.is_metro" style="padding: 1px 6px; border-radius: 4px; font-size: 9px; font-weight: 800; background: #fef3c7; color: #d97706; text-transform: uppercase;">Metro</span>
                      </div>
                      <p style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0; font-family: monospace;">{{ row.city_code }}</p>
                    </div>
                  </td>
                  <td style="padding: 14px 16px; color: #6b7280;">{{ row.state?.name || '-' }}</td>
                  <td style="padding: 14px 16px; color: #6b7280; display: none;" class="lg-cell">{{ row.country?.name || '-' }}</td>
                  ...
                  <td style="padding: 14px 16px; text-align: center; position: relative;" (click)="$event.stopPropagation()">
                    ...
                    <div *ngIf="openMenuId === row.id"
                      ...
                      <button *ngIf="!row.is_default" (click)="deleteCity(row)"
      ...
      <div *ngIf="importing" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 50; animation: fadeIn 0.2s;">
        <div style="background: white; border-radius: 20px; padding: 32px; text-align: center; box-shadow: 0 24px 48px rgba(0,0,0,0.2); max-width: 320px; width: calc(100% - 32px);">
          ...
          <p style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">Importing cities...</p>
        ...
        <div (click)="$event.stopPropagation()" style="background: white; border-radius: 20px; padding: 24px; max-width: 480px; width: calc(100% - 32px); box-shadow: 0 24px 48px rgba(0,0,0,0.2);">
          ...
          <div style="margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 8px; font-size: 13px; margin-bottom: 8px;">
              ...
              <span style="color: #374151;">{{ importResult.success_count }} cities imported successfully</span>
            </div>
...
export class CitiesComponent implements OnInit {
  ...
  countries: Country[] = [];
  states: State[] = [];
  selectedCities = new Set<City>();
  openMenuId: number | null = null;

  loading = false;
  importing = false;
  importResult: CityImportResult | null = null;
  ...
  loadCities(): void {
    ...
    this.cityApi.getAll(params).subscribe({
      next: (res) => {
        this.cities = res.data || [];
        this.totalCount = this.cities.length;
        this.totalPages = Math.ceil(this.totalCount / this.perPage) || 1;
        this.activeCount = this.cities.filter(c => c.status === 'active').length;
        this.metroCount = this.cities.filter(c => c.is_metro).length;
        this.defaultCount = this.cities.filter(c => c.is_default).length;
        this.updateStatCards();
        this.loading = false;
      },
      error: () => { this.loading = false; this.notification.error('Failed to load cities'); },
    });
  }

  updateStatCards(): void {
    this.statCards = [
      { label: 'Total Cities', value: this.totalCount, icon: 'location_city', bg: '#d1fae5', color: '#047857' },
      { label: 'Active', value: this.activeCount, icon: 'check_circle', bg: '#a7f3d0', color: '#059669' },
      { label: 'Metro Cities', value: this.metroCount, icon: 'train', bg: '#fef3c7', color: '#d97706' },
  ...
  onSearchDebounce(): void {
    ...
    this.searchTimeout = setTimeout(() => { this.currentPage = 1; this.loadCities(); }, 400);
  }

  clearSearch(): void { this.search = ''; this.currentPage = 1; this.loadCities(); }
  onFilterChange(): void { this.currentPage = 1; this.loadCities(); }
  clearFilters(): void { this.search = ''; this.statusFilter = null; this.countryFilter = null; this.stateFilter = null; this.states = []; this.currentPage = 1; this.loadCities(); }
  ...
  isAllSelected(): boolean {
    return this.cities.length > 0 && this.selectedCities.size === this.cities.length;
  }

  toggleAllRows(event: any): void {
    if (event.target.checked) {
      this.cities.forEach(c => this.selectedCities.add(c));
    } else {
  ...
  toggleRow(row: City): void {
    if (this.selectedCities.has(row)) {
      this.selectedCities.delete(row);
    } else {
      this.selectedCities.add(row);
    }
    this.openMenuId = null;
  }

  clearSelection(): void { this.selectedCities.clear(); }

  toggleMenu(row: City): void {
  ...
  viewCity(row: City): void { this.router.navigate(['/admin/cities', row.uuid]); }
  editCity(row: City): void { this.router.navigate(['/admin/cities', row.uuid, 'edit']); }

  setDefault(row: City): void {
    this.openMenuId = null;
    if (window.confirm(`Set "${row.name}" as the default city?`)) {
      this.cityApi.setDefault(row.uuid).subscribe({
        next: () => { this.notification.success('Default city updated'); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  setStatus(row: City, status: string): void {
    this.openMenuId = null;
    const label = status === 'active' ? 'Activate' : 'Deactivate';
    if (window.confirm(`${label} "${row.name}"?`)) {
      this.cityApi.setStatus(row.uuid, status).subscribe({
        next: () => { this.notification.success(`City ${status === 'active' ? 'activated' : 'deactivated'}`); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  deleteCity(row: City): void {
    this.openMenuId = null;
    if (window.confirm(`Delete "${row.name}"? This action cannot be undone.`)) {
      this.cityApi.delete(row.uuid).subscribe({
        next: () => { this.notification.success('City deleted'); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetActive(): void {
    const ids = [...this.selectedCities].map(r => r.id);
    if (window.confirm(`Activate ${ids.length} selected cities?`)) {
      this.cityApi.bulkStatus(ids, 'active').subscribe({
        next: () => { this.notification.success('Cities activated'); this.clearSelection(); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetInactive(): void {
    const ids = [...this.selectedCities].map(r => r.id);
    if (window.confirm(`Deactivate ${ids.length} selected cities?`)) {
      this.cityApi.bulkStatus(ids, 'inactive').subscribe({
        next: () => { this.notification.success('Cities deactivated'); this.clearSelection(); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkDelete(): void {
    const ids = [...this.selectedCities].map(r => r.id);
    if (window.confirm(`Delete ${ids.length} selected cities? This cannot be undone.`)) {
      this.cityApi.bulkDelete(ids).subscribe({
        next: () => { this.notification.success('Cities deleted'); this.clearSelection(); this.loadCities(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  onFileSelected(event: Event): void {
    ...
    this.importing = true;
    this.cityApi.import(file).subscribe({
      next: (res) => { this.importing = false; this.importResult = res.data || null; this.loadCities(); input.value = ''; },
  ...
  exportCities(): void {
    ...
    this.cityApi.export(filters).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'cities-export.csv'; a.click();

...
import { MenuTemplate } from '../../core/models/monthly-menu/menu-template.model';
  ...
  template: `
    ...
    <div *ngIf="!loading || templates.length > 0" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Menu Templates</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Menu Templates</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Manage reusable menu templates for your kitchens</p>
        </div>
        <a routerLink="/admin/menu-templates/create"
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
          <div style="position: relative; flex: 1; min-width: 200px;">
            ...
            <input type="text" [(ngModel)]="search" (ngModelChange)="onSearchDebounce()" placeholder="Search templates..."
          ...
          <select [(ngModel)]="isDefaultFilter" (change)="onFilterChange()"
            ...
            <option [ngValue]="true">Default Only</option>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden;">
        ...
        <div *ngIf="!loading && templates.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No menu templates found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 280px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || kitchenFilter || isDefaultFilter !== null">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !kitchenFilter && isDefaultFilter === null">Get started by creating your first menu template</span>
          </p>
          <a *ngIf="!search && !statusFilter && !kitchenFilter && isDefaultFilter === null" routerLink="/admin/menu-templates/create"
          ...
          <button *ngIf="search || statusFilter || kitchenFilter || isDefaultFilter !== null" (click)="clearFilters()"
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 200px;">Template Name</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; display: none;" [style.display]="'table-cell'" class="lg-cell">Description</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; display: none;" [style.display]="'table-cell'" class="md-cell">Kitchen</th>
                ...
                <th style="padding: 10px 12px; text-align: center; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; display: none;" [style.display]="'table-cell'" class="md-cell">Default</th>
            ...
            <tbody>
              <tr *ngFor="let row of visibleTemplates; let i = index" (click)="viewTemplate(row)" style="cursor: pointer; border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px;">
                  <div>
                    <p style="font-size: 13px; font-weight: 700; color: #1f2937; margin: 0;">{{ row.template_name }}</p>
                ...
                <td style="padding: 12px 12px; display: none;" [style.display]="'table-cell'" class="lg-cell">
                  <span style="font-size: 12px; color: #9ca3af; max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: block;">{{ row.description || '-' }}</span>
                </td>
                ...
                <td style="padding: 12px 12px; text-align: center;">
                  <span style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 8px; background: #d1fae5; font-size: 12px; font-weight: 700; color: #047857;">{{ row.items_count || 0 }}</span>
                </td>
                <td style="padding: 12px 12px; text-align: center; display: none;" [style.display]="'table-cell'" class="md-cell">
                  <span *ngIf="row.is_default" style="padding: 3px 10px; border-radius: 20px; font-size: 10px; font-weight: 800; background: #fef3c7; color: #b45309; display: inline-flex; align-items: center; gap: 4px; text-transform: uppercase; letter-spacing: 0.03em;">
                    <span class="material-icons" style="font-size: 14px;">star</span> Default
                  </span>
                  <span *ngIf="!row.is_default" style="font-size: 12px; color: #d1d5db;">-</span>
                </td>
                <td style="padding: 12px 12px; text-align: center;">
                ...
                <td style="padding: 12px 16px 12px 12px; text-align: center;">
                  <div style="display: flex; align-items: center; justify-content: center; gap: 2px;">
                    <button (click)="viewTemplate(row); $event.stopPropagation()"
                    ...
                    <div style="position: relative;">
                      ...
                      <div *ngIf="activeActionRow?.uuid === row.uuid" style="position: absolute; right: 0; top: 100%; z-index: 50; background: white; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 10px 40px rgba(0,0,0,0.12); min-width: 180px; padding: 6px; margin-top: 4px; animation: fadeIn 0.1s ease-out;">
                        <button (click)="viewTemplate(row); toggleActionMenu(null); $event.stopPropagation()"
                        ...
                        <button (click)="duplicateTemplate(row); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
...
export class MenuTemplatesComponent implements OnInit {
  ...
  selectedRows = new Set<MenuTemplate>();
  activeActionRow: MenuTemplate | null = null;
  // displayedColumns removed - no longer needed
  ...
  isDefaultFilter: boolean | null = null;
  ...
  get visibleTemplates(): MenuTemplate[] {
  ...
  loadTemplates(): void {
    ...
    if (this.isDefaultFilter !== null) params['is_default'] = this.isDefaultFilter.toString();

    this.menuTemplateApi.getMenuTemplatePaginated(params).subscribe({
      next: (res) => {
      ...
      error: () => { this.loading = false; this.notification.error('Failed to load menu templates'); },
    });
  }

  onSearchDebounce(): void {
    clearTimeout(this.searchTimeout);
    this.searchTimeout = setTimeout(() => { this.currentPage = 1; this.loadTemplates(); }, 400);
  }

  clearSearch(): void { this.search = ''; this.currentPage = 1; this.loadTemplates(); }
  onFilterChange(): void { this.currentPage = 1; this.loadTemplates(); }
  clearFilters(): void {
    this.search = '';
    this.statusFilter = null;
    this.kitchenFilter = null;
    this.isDefaultFilter = null;
    this.currentPage = 1;
    this.loadTemplates();
  }
  ...
  isAllSelected(): boolean { return this.templates.length > 0 && this.selectedRows.size === this.templates.length; }

  toggleAllRows(event: any): void {
    if (event.target.checked) { this.templates.forEach(t => this.selectedRows.add(t)); } else { this.selectedRows.clear(); }
    this.activeActionRow = null;
  }

  toggleRow(row: MenuTemplate): void {
  ...
  toggleActionMenu(row: MenuTemplate | null): void {
  ...
  viewTemplate(row: MenuTemplate): void { this.activeActionRow = null; this.router.navigate(['/admin/menu-templates', row.uuid]); }
  editTemplate(row: MenuTemplate): void { this.activeActionRow = null; this.router.navigate(['/admin/menu-templates', row.uuid, 'edit']); }

  duplicateTemplate(row: MenuTemplate): void {
    if (window.confirm(`Duplicate "${row.template_name}"?`)) {
      this.menuTemplateApi.duplicateMenuTemplate(row.uuid).subscribe({
        next: () => { this.notification.success('Template duplicated'); this.loadTemplates(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  setDefault(row: MenuTemplate): void {
    if (window.confirm(`Set "${row.template_name}" as default template?`)) {
      this.menuTemplateApi.setDefault(row.uuid).subscribe({
        next: () => { this.notification.success('Template set as default'); this.loadTemplates(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  deleteTemplate(row: MenuTemplate): void {
    if (window.confirm(`Delete "${row.template_name}"? This action cannot be undone.`)) {
      this.menuTemplateApi.deleteMenuTemplate(row.uuid).subscribe({
        next: () => { this.notification.success('Template deleted'); this.loadTemplates(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkDelete(): void {
    ...
    if (window.confirm(`Delete ${ids.length} selected templates? This cannot be undone.`)) {
      let completed = 0;
      ids.forEach((template) => {
        this.menuTemplateApi.deleteMenuTemplate(template.uuid).subscribe({
          next: () => { completed++; if (completed === ids.length) { this.notification.success('Templates deleted'); this.clearSelection(); this.loadTemplates(); } },
          error: () => { completed++; if (completed === ids.length) { this.notification.success('Templates deleted'); this.clearSelection(); this.loadTemplates(); } },
        });
      });
    }
  }

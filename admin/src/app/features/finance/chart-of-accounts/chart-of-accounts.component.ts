  ...
  template: `
    <div *ngIf="loading && accounts.length === 0" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        ...
        <p style="font-size: 13px; color: #9ca3af;">Loading chart of accounts...</p>
      </div>
    </div>
    ...
    <div *ngIf="!loading || accounts.length > 0" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span class="material-icons" style="font-size: 14px; color: #d1d5db;">chevron_right</span>
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Chart of Accounts</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Chart of Accounts</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Manage your accounts hierarchy, balances, and classifications</p>
        </div>
        <a routerLink="/admin/finance/accounts/create"
          ...
          <span class="material-icons" style="font-size: 18px;">add</span> Create Account
        </a>
      </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px;">
        ...
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          ...
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #dbeafe; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #1d4ed8;">trending_up</span>
          </div>
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Assets Total</p>
            <p style="font-size: 22px; font-weight: 800; color: #166534; margin: 0;">{{ formatCurrency(assetsTotal) }}</p>
          </div>
        </div>
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,0.05)';this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #fee2e2; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #b91c1c;">trending_down</span>
          </div>
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Liabilities Total</p>
            <p style="font-size: 22px; font-weight: 800; color: #166534; margin: 0;">{{ formatCurrency(liabilitiesTotal) }}</p>
          </div>
        </div>
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,0.05)';this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #d1fae5; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #047857;">savings</span>
          </div>
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Income Total</p>
            <p style="font-size: 22px; font-weight: 800; color: #166534; margin: 0;">{{ formatCurrency(incomeTotal) }}</p>
          </div>
        </div>
      </div>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
          <div style="position: relative; flex: 1; min-width: 200px;">
            ...
            <input type="text" [(ngModel)]="searchTerm" (keyup.enter)="loadAccounts()" placeholder="Search accounts..."
          ...
          <select [(ngModel)]="filterType" (change)="loadAccounts()"
            ...
            <option *ngFor="let t of accountTypes" [ngValue]="t.value">{{ t.label }}</option>
          </select>
        </div>
      </div>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden;">
        ...
        <div *ngIf="!loading && filteredAccounts.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No accounts found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 280px; margin: 0 0 24px 0;">
            <span *ngIf="searchTerm || filterType">Try adjusting your search or filter criteria</span>
            <span *ngIf="!searchTerm && !filterType">Get started by creating your first account</span>
          </p>
          <a *ngIf="!searchTerm && !filterType" routerLink="/admin/finance/accounts/create"
          ...
          <button *ngIf="searchTerm || filterType" (click)="searchTerm = ''; filterType = ''; loadAccounts()"
        ...
        <div *ngIf="!loading && filteredAccounts.length > 0" style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                <th style="padding: 10px 12px 10px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 90px;">Code</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 220px;">Name</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 100px;">Type</th>
                <th style="padding: 10px 12px; text-align: right; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 130px;">Current Balance</th>
            ...
            <tbody>
              <tr *ngFor="let row of paginatedAccounts; let i = index" style="border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td [attr.data-idx]="i" style="padding: 12px 12px 12px 16px;">
                  <span style="font-size: 12px; font-weight: 700; font-family: monospace; color: #059669;">{{ row.account_code }}</span>
                </td>
                <td style="padding: 12px 12px;">
                  <p style="font-size: 13px; font-weight: 600; color: #1f2937; margin: 0;">{{ row.account_name }}</p>
                  <p *ngIf="row.remarks" style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0;">{{ row.remarks }}</p>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                    [style.background]="row.account_type === 'asset' ? '#dbeafe' : row.account_type === 'liability' ? '#fee2e2' : row.account_type === 'equity' ? '#ede9fe' : row.account_type === 'income' ? '#d1fae5' : '#ffedd5'"
                    [style.color]="row.account_type === 'asset' ? '#1d4ed8' : row.account_type === 'liability' ? '#b91c1c' : row.account_type === 'equity' ? '#7c3aed' : row.account_type === 'income' ? '#047857' : '#9a3412'">
                    {{ getTypeLabel(row.account_type) }}
                  </span>
                </td>
                <td style="padding: 12px 12px; text-align: right;">
                  <span style="font-size: 13px; font-weight: 700; color: #1f2937;">₹{{ formatNumber(Math.abs(row.current_balance)) }}</span>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                    [style.background]="row.status === 'active' ? '#d1fae5' : '#fee2e2'"
                    [style.color]="row.status === 'active' ? '#047857' : '#b91c1c'">
...
export class ChartOfAccountsComponent implements OnInit {
  ...
  currentPage = 1;
  pageSize = 15;
  activeActionRow: ChartOfAccount | null = null;

  accountTypes = ACCOUNT_TYPES;

  get filteredAccounts(): ChartOfAccount[] {
    ...
    if (this.filterType) {
      result = result.filter(a => a.account_type === this.filterType);
    }
    return result;
  }

  get paginatedAccounts(): ChartOfAccount[] {
  ...
  get assetsTotal(): number {
    return this.accounts.filter(a => a.account_type === 'asset').reduce((sum, a) => sum + a.current_balance, 0);
  }

  get liabilitiesTotal(): number {
    return this.accounts.filter(a => a.account_type === 'liability').reduce((sum, a) => sum + a.current_balance, 0);
  }

  get incomeTotal(): number {
    return this.accounts.filter(a => a.account_type === 'income').reduce((sum, a) => sum + a.current_balance, 0);
  }
  ...
  loadAccounts(): void {
    ...
    this.financeApi.getAccounts(params).subscribe({
      next: (res) => {
  ...
  getRangeLabel(): string {
    if (this.filteredAccounts.length === 0) return '0 of 0';
    const start = (this.currentPage - 1) * this.pageSize + 1;
    const end = Math.min(this.currentPage * this.pageSize, this.filteredAccounts.length);
    return `${start}\u2013${end} of ${this.filteredAccounts.length}`;
  }

  toggleActionMenu(row: ChartOfAccount | null): void {
    this.activeActionRow = this.activeActionRow?.uuid === row?.uuid ? null : row;
  }

  createAccount(): void {
    this.router.navigate(['/admin/finance/accounts/create']);
  }

  editAccount(account: ChartOfAccount): void {
    this.activeActionRow = null;
    this.router.navigate(['/admin/finance/accounts', account.uuid, 'edit']);
  }

  deleteAccount(account: ChartOfAccount): void {
    this.activeActionRow = null;
    if (confirm(`Are you sure you want to delete ${account.account_name}?`)) {
      this.financeApi.deleteAccount(account.uuid).subscribe({
        next: () => {

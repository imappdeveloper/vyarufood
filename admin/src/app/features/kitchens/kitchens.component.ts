...
import { Kitchen, KitchenImportResult } from '../../core/models/kitchen/kitchen.model';
...
@Component({
  ...
  template: `
    <div *ngIf="loading && kitchens.length === 0" style="display: flex; align-items: center; justify-content: center; min-height: 60vh;">
      <div style="text-align: center;">
        <div style="width: 48px; height: 48px; border: 3px solid #e5e7eb; border-top-color: #059669; border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto 16px;"></div>
        <p style="font-size: 13px; color: #9ca3af;">Loading kitchens...</p>
      </div>
    </div>

    <div *ngIf="!loading || kitchens.length > 0" style="animation: fadeIn 0.3s ease-out; max-width: 1280px; margin: 0 auto; padding: 0 24px 60px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
        ...
        <span style="font-size: 12px; color: #374151; font-weight: 600;">Kitchens</span>
      </div>

      <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Kitchens</h1>
          <p style="font-size: 13px; color: #9ca3af; margin: 0;">Manage kitchen locations and settings</p>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <a routerLink="/admin/kitchens/create"
            ...
            <span class="material-icons" style="font-size: 18px;">add</span> Add Kitchen
          </a>
        </div>
      </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px;">
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          ...
          <div>
            <p style="font-size: 11px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 2px 0;">Total Kitchens</p>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
          <div style="position: relative; flex: 1; min-width: 200px;">
            ...
            <input type="text" [(ngModel)]="search" (keyup.enter)="loadKitchens()" (ngModelChange)="onSearchDebounce()" placeholder="Search kitchens..."
          ...
          <div style="position: relative;">
            <select [(ngModel)]="kitchenTypeFilter" (change)="onFilterChange()"
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden;">
        <div *ngIf="loading && kitchens.length > 0" style="display: flex; align-items: center; justify-content: center; padding: 32px;">
        ...
        <div *ngIf="!loading && kitchens.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          <div style="width: 72px; height: 72px; border-radius: 20px; background: linear-gradient(135deg, #d1fae5, #a7f3d0); display: flex; align-items: center; justify-content: center; margin-bottom: 20px;">
            <span class="material-icons" style="font-size: 32px; color: #059669;">restaurant</span>
          </div>
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No kitchens found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 280px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || kitchenTypeFilter || defaultFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !kitchenTypeFilter && !defaultFilter">Get started by adding your first kitchen</span>
          </p>
          <a *ngIf="!search && !statusFilter && !kitchenTypeFilter && !defaultFilter" routerLink="/admin/kitchens/create"
          ...
          <button *ngIf="search || statusFilter || kitchenTypeFilter || defaultFilter" (click)="clearFilters()"
        ...
        <div *ngIf="!loading && kitchens.length > 0" style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              ...
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                <th style="padding: 10px 8px 10px 16px; text-align: left; width: 44px;">
                  <input type="checkbox" (change)="toggleAllRows($event)"
                    [checked]="selectedRows.length === kitchens.length && kitchens.length > 0"
                    style="width: 16px; height: 16px; accent-color: #059669; cursor: pointer; margin: 0;" />
                </th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; min-width: 200px;">Kitchen</th>
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; display: none;" [style.display]="'table-cell'" class="lg-cell">City</th>
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; display: none;" [style.display]="'table-cell'" class="xl-cell">Area</th>
            ...
            <tbody>
              <tr *ngFor="let row of displayedKitchens; let i = index" (click)="viewKitchen(row)" style="cursor: pointer; border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td [attr.data-idx]="i" style="padding: 12px 8px 12px 16px;">
                  <input type="checkbox" (click)="$event.stopPropagation()" (change)="toggleRow(row)"
                    [checked]="selection.has(row)" style="width: 16px; height: 16px; accent-color: #059669; cursor: pointer; margin: 0;" />
                </td>
                <td style="padding: 12px 12px;">
                  <div>
                    ...
                    <p style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0;">{{ row.kitchen_code }}</p>
                  </div>
                </td>
                <td style="padding: 12px 12px;">
                  <span style="display: inline-flex; padding: 3px 10px; border-radius: 6px; font-size: 11px; font-weight: 700; background: #d1fae5; color: #047857;">
                    {{ row.kitchen_type_label }}
                  </span>
                </td>
                <td style="padding: 12px 12px; display: none;" [style.display]="'table-cell'" class="lg-cell">
                  <span style="font-size: 13px; color: #6b7280;">{{ row.city?.name || '-' }}</span>
                </td>
                <td style="padding: 12px 12px; display: none;" [style.display]="'table-cell'" class="xl-cell">
                  <span style="font-size: 13px; color: #6b7280;">{{ row.area?.name || '-' }}</span>
                </td>
                ...
                <td style="padding: 12px 16px 12px 12px; text-align: center;">
                  <div style="display: flex; align-items: center; justify-content: center; gap: 2px;">
                    <button (click)="viewKitchen(row); $event.stopPropagation()"
                    ...
                    <div style="position: relative;" #actionMenuContainer>
                      ...
                      <div *ngIf="activeActionRow?.uuid === row.uuid" style="position: absolute; right: 0; top: 100%; z-index: 50; background: white; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 10px 40px rgba(0,0,0,0.12); min-width: 180px; padding: 6px; margin-top: 4px; animation: fadeIn 0.1s ease-out;">
                        <button (click)="viewKitchen(row); toggleActionMenu(null); $event.stopPropagation()"
                        ...
                        <button (click)="editKitchen(row); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                        ...
                        <div style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                        <button (click)="deleteKitchen(row); toggleActionMenu(null); $event.stopPropagation()"
          ...
          <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; border-top: 1px solid #f3f4f6; flex-wrap: wrap; gap: 8px;">
            <span style="font-size: 12px; color: #9ca3af;">Showing {{ (currentPage - 1) * perPage + 1 }}–{{ Math.min(currentPage * perPage, totalCount) }} of {{ totalCount }}</span>
            <div style="display: flex; align-items: center; gap: 6px;">
      ...
      <div *ngIf="importing" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000; animation: fadeIn 0.15s ease-out;">
        <div style="background: white; border-radius: 16px; padding: 32px; text-align: center; max-width: 360px; width: 90vw; box-shadow: 0 20px 60px rgba(0,0,0,0.2);">
          ...
          <p style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">Importing kitchens...</p>
        ...
        <div style="background: white; border-radius: 16px; padding: 24px; max-width: 420px; width: 90vw; box-shadow: 0 20px 60px rgba(0,0,0,0.2);" (click)="$event.stopPropagation()">
          ...
          <div style="margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 8px; padding: 8px 0;">
              ...
              <span style="font-size: 13px; color: #374151;">{{ importResult.successes }} kitchens imported successfully</span>
            </div>
...
export class KitchensComponent implements OnInit {
  ...
  kitchens: Kitchen[] = [];
  selection = new Set<Kitchen>();

  loading = false;
  importing = false;
  importResult: KitchenImportResult | null = null;

  currentPage = 1;
  perPage = 10;
  search = '';
  statusFilter: string | null = null;
  kitchenTypeFilter: string | null = null;
  ...
  activeActionRow: Kitchen | null = null;
  private searchTimeout: any;

  Math = Math;

  get displayedKitchens(): Kitchen[] {
    const start = (this.currentPage - 1) * this.perPage;
    return this.kitchens.slice(start, start + this.perPage);
  }
  ...
  loadKitchens(): void {
    ...
    if (this.kitchenTypeFilter) params['kitchen_type'] = this.kitchenTypeFilter;
    if (this.defaultFilter) params['is_default'] = this.defaultFilter;

    this.kitchenApi.getAll(params).subscribe({
      next: (res) => {
        this.kitchens = res.data || [];
        this.totalCount = this.kitchens.length;
        this.activeCount = this.kitchens.filter(k => k.status === 'active').length;
        this.inactiveCount = this.kitchens.filter(k => k.status === 'inactive').length;
        this.defaultCount = this.kitchens.filter(k => k.is_default).length;
        this.loading = false;
      },
      error: () => { this.loading = false; this.notification.error('Failed to load kitchens'); },
    });
  }

  onSearchDebounce(): void {
    clearTimeout(this.searchTimeout);
    this.searchTimeout = setTimeout(() => { this.currentPage = 1; this.loadKitchens(); }, 400);
  }

  clearSearch(): void { this.search = ''; this.currentPage = 1; this.loadKitchens(); }
  onFilterChange(): void { this.currentPage = 1; this.loadKitchens(); }
  clearFilters(): void {
    this.search = '';
    this.statusFilter = null;
    this.kitchenTypeFilter = null;
  ...
  toggleAllRows(event: Event): void {
    ...
    if (checked) {
      this.kitchens.forEach(k => this.selection.add(k));
    } else {
      this.selection.clear();
    }
  }

  toggleRow(row: Kitchen): void {
  ...
  toggleActionMenu(row: Kitchen | null): void {
    this.activeActionRow = this.activeActionRow?.uuid === row?.uuid ? null : row;
  }

  viewKitchen(row: Kitchen): void {
    this.activeActionRow = null;
    this.router.navigate(['/admin/kitchens', row.uuid]);
  }

  editKitchen(row: Kitchen): void {
    this.activeActionRow = null;
    this.router.navigate(['/admin/kitchens', row.uuid, 'edit']);
  }

  setDefault(row: Kitchen): void {
    if (window.confirm(`Set "${row.name}" as default kitchen?`)) {
      this.kitchenApi.setDefault(row.uuid).subscribe({
        next: () => { this.notification.success('Default kitchen updated'); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  setStatus(row: Kitchen, status: string): void {
    const label = status === 'active' ? 'Activate' : 'Deactivate';
    if (window.confirm(`${label} "${row.name}"?`)) {
      this.kitchenApi.setStatus(row.uuid, status).subscribe({
        next: () => { this.notification.success(`Kitchen ${status === 'active' ? 'activated' : 'deactivated'}`); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  deleteKitchen(row: Kitchen): void {
    if (row.is_default) {
      this.notification.error('Default kitchen cannot be deleted');
      return;
    }
    if (window.confirm(`Delete "${row.name}"? This action cannot be undone.`)) {
      this.kitchenApi.delete(row.uuid).subscribe({
        next: () => { this.notification.success('Kitchen deleted'); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetActive(): void {
    const ids = Array.from(this.selection).map(r => r.id);
    if (window.confirm(`Activate ${ids.length} selected kitchens?`)) {
      this.kitchenApi.bulkSetStatus(ids, 'active').subscribe({
        next: () => { this.notification.success('Kitchens activated'); this.clearSelection(); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkSetInactive(): void {
    const ids = Array.from(this.selection).map(r => r.id);
    if (window.confirm(`Deactivate ${ids.length} selected kitchens?`)) {
      this.kitchenApi.bulkSetStatus(ids, 'inactive').subscribe({
        next: () => { this.notification.success('Kitchens deactivated'); this.clearSelection(); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  bulkDelete(): void {
    const ids = Array.from(this.selection).map(r => r.id);
    if (window.confirm(`Delete ${ids.length} selected kitchens? This cannot be undone.`)) {
      this.kitchenApi.bulkDelete(ids).subscribe({
        next: () => { this.notification.success('Kitchens deleted'); this.clearSelection(); this.loadKitchens(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }
  ...
  onFileSelected(event: Event): void {
    ...
    this.importing = true;
    this.kitchenApi.import(file).subscribe({
      next: (res) => { this.importing = false; this.importResult = res.data || null; this.loadKitchens(); input.value = ''; },
  ...
  exportKitchens(): void {
    ...
    if (this.kitchenTypeFilter) filters['kitchen_type'] = this.kitchenTypeFilter;
    this.kitchenApi.export(filters).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'kitchens-export.csv'; a.click();

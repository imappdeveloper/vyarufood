...
import { MonthlyMenu, MonthlyMenuStats } from '../../core/models/monthly-menu/monthly-menu.model';

@Component({
  selector: 'app-monthly-menus',
  standalone: true,
  ...
  template: `
    ...
    <div *ngIf="!loading || menus.length > 0" style="animation: fadeIn 0.3s ease-out;">
      <div style="margin-bottom: 32px;">
        <nav style="display: flex; align-items: center; gap: 8px; font-size: 12px; color: #9ca3af; margin-bottom: 4px;">
          ...
          <span style="font-size: 12px; color: #374151; font-weight: 600;">Monthly Menus</span>
        </nav>
        <div style="display: flex; align-items: flex-start; justify-content: space-between;">
          <div>
            <h1 style="font-size: 24px; font-weight: 800; color: #166534; margin: 8px 0 4px 0;">Monthly Menu Planner</h1>
            <p style="font-size: 13px; color: #9ca3af; margin: 0;">Plan and manage monthly menus for your kitchens</p>
          </div>
          <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <a routerLink="/admin/monthly-menus/create"
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px;">
        ...
        <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 20px; display: flex; align-items: center; gap: 14px; transition: all 0.2s ease;"
          ...
          <div style="width: 44px; height: 44px; border-radius: 12px; background: #dbeafe; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span class="material-icons" style="font-size: 22px; color: #2563eb;">verified</span>
          </div>
          <div>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; padding: 16px; margin-bottom: 16px;">
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
          ...
          <select [(ngModel)]="monthFilter" (change)="onFilterChange()"
          ...
          <select [(ngModel)]="yearFilter" (change)="onFilterChange()"
            style="padding: 9px 32px 9px 12px; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 13px; color: #374151; background: #f9fafb; outline: none; cursor: pointer; appearance: none; -webkit-appearance: none; min-width: 110px; transition: all 0.2s ease;"
            onfocus="this.style.borderColor='#059669';this.style.boxShadow='0 0 0 3px rgba(5,150,105,0.1)'" onblur="this.style.borderColor='#e5e7eb';this.style.boxShadow=''">
      ...
      <div *ngIf="selectedMenus.size > 0" style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; margin-bottom: 16px; padding: 12px 20px; display: flex; align-items: center; gap: 12px; animation: slideDown 0.2s ease-out;">
        <span style="font-size: 13px; font-weight: 700; color: #047857;">{{ selectedMenus.size }} selected</span>
      ...
      <div style="background: white; border-radius: 14px; border: 1px solid #e5e7eb; overflow: hidden;">
        ...
        <div *ngIf="!loading && menus.length === 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px;">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #166534; margin: 0 0 4px 0;">No monthly menus found</h3>
          <p style="font-size: 13px; color: #9ca3af; text-align: center; max-width: 280px; margin: 0 0 24px 0;">
            <span *ngIf="search || statusFilter || kitchenFilter || monthFilter || yearFilter">Try adjusting your search or filter criteria</span>
            <span *ngIf="!search && !statusFilter && !kitchenFilter && !monthFilter && !yearFilter">Get started by creating your first monthly menu</span>
          </p>
          <a *ngIf="!search && !statusFilter && !kitchenFilter && !monthFilter && !yearFilter" routerLink="/admin/monthly-menus/create"
          ...
          <button *ngIf="search || statusFilter || kitchenFilter || monthFilter || yearFilter" (click)="clearFilters()"
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Month / Year</th>
                ...
                <th style="padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;" class="md-cell">Kitchen</th>
                <th style="padding: 10px 12px; text-align: center; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;" class="lg-cell">Items</th>
            ...
            <tbody>
              <tr *ngFor="let row of visibleMenus; let i = index" (click)="viewMenu(row)" style="cursor: pointer; border-bottom: 1px solid #f3f4f6; transition: background 0.1s ease;"
                ...
                <td style="padding: 12px 12px;">
                  <div>
                    <p style="font-weight: 700; color: #1f2937; margin: 0; line-height: 1.2;">{{ row.title }}</p>
                    <p *ngIf="row.description" style="font-size: 11px; color: #9ca3af; margin: 2px 0 0 0; max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ row.description }}</p>
                  </div>
                </td>
                <td style="padding: 12px 12px; font-size: 13px; color: #374151;">{{ formatMonthYear(row.month, row.year) }}</td>
                <td style="padding: 12px 12px; text-align: center;">
                  <span style="display: inline-flex; padding: 3px 12px; border-radius: 20px; font-size: 11px; font-weight: 700;"
                    [style.background]="row.status === 'published' ? '#d1fae5' : row.status === 'draft' ? '#fef3c7' : row.status === 'approved' ? '#dbeafe' : '#f3f4f6'"
                    [style.color]="row.status === 'published' ? '#047857' : row.status === 'draft' ? '#b45309' : row.status === 'approved' ? '#1d4ed8' : '#6b7280'">
                    {{ row.status | titlecase }}
                  </span>
                </td>
                <td style="padding: 12px 12px; font-size: 13px; color: #6b7280;" class="md-cell">{{ row.kitchen_name || '-' }}</td>
                <td style="padding: 12px 12px; text-align: center;" class="lg-cell">
                  <span style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 8px; background: #d1fae5; font-size: 12px; font-weight: 700; color: #047857;">{{ row.items?.length || row.items_count || 0 }}</span>
                </td>
                  ...
                  <div style="display: flex; align-items: center; justify-content: center; gap: 2px;">
                    ...
                    <div style="position: relative;">
                      ...
                      <div *ngIf="activeActionRow?.uuid === row.uuid" style="position: absolute; right: 0; top: 100%; z-index: 50; background: white; border-radius: 12px; border: 1px solid #e5e7eb; box-shadow: 0 10px 40px rgba(0,0,0,0.12); min-width: 180px; padding: 6px; margin-top: 4px; animation: fadeIn 0.1s ease-out;">
                        ...
                        <button *ngIf="row.status !== 'published' && row.status !== 'approved'" (click)="publishMenu(row); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                        ...
                        <button *ngIf="row.status === 'published'" (click)="approveMenu(row); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                          <span class="material-icons" style="font-size: 18px; color: #2563eb;">verified</span> Approve
                        </button>
                        <div style="height: 1px; background: #f3f4f6; margin: 4px 0;"></div>
                        <button (click)="duplicateMenu(row); toggleActionMenu(null); $event.stopPropagation()"
                          style="display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 12px; border: none; background: none; cursor: pointer; font-size: 13px; color: #374151; border-radius: 8px; text-align: left; transition: background 0.1s ease;"
                          onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
...
export class MonthlyMenusComponent implements OnInit {
  ...
  menus: MonthlyMenu[] = [];
  selectedMenus = new Set<MonthlyMenu>();
  activeActionRow: MonthlyMenu | null = null;
  ...
  totalCount = 0;
  stats: MonthlyMenuStats | null = null;
  kitchens: any[] = [];
  ...
  loadStats(): void {
    this.monthlyMenuApi.getMonthlyMenuStats().subscribe({
  ...
  loadMenus(): void {
    ...
    if (this.monthFilter) params['month'] = this.monthFilter.toString();
    if (this.yearFilter) params['year'] = this.yearFilter.toString();

    this.monthlyMenuApi.getMonthlyMenuPaginated(params).subscribe({
      next: (res) => {
        this.menus = res.data || [];
      ...
      error: () => { this.loading = false; this.notification.error('Failed to load monthly menus'); },
    });
  }
  ...
  toggleRow(row: MonthlyMenu): void {
  ...
  toggleActionMenu(row: MonthlyMenu | null): void {
  ...
  viewMenu(row: MonthlyMenu): void { this.router.navigate(['/admin/monthly-menus', row.uuid]); }
  editMenu(row: MonthlyMenu): void { this.router.navigate(['/admin/monthly-menus', row.uuid, 'edit']); }

  publishMenu(row: MonthlyMenu): void {
    if (window.confirm(`Publish "${row.title}"?`)) {
      this.monthlyMenuApi.publishMonthlyMenu(row.uuid).subscribe({
  ...
  unpublishMenu(row: MonthlyMenu): void {
    if (window.confirm(`Unpublish "${row.title}"?`)) {
      this.monthlyMenuApi.unpublishMonthlyMenu(row.uuid).subscribe({
  ...
  approveMenu(row: MonthlyMenu): void {
    if (window.confirm(`Approve "${row.title}"?`)) {
      this.monthlyMenuApi.approveMonthlyMenu(row.uuid).subscribe({
        next: () => { this.notification.success('Menu approved'); this.loadMenus(); this.loadStats(); },
        error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
      });
    }
  }

  duplicateMenu(row: MonthlyMenu): void {
    const targetMonth = window.prompt('Enter target month (1-12):');
    if (!targetMonth) return;
    const targetYear = window.prompt('Enter target year (e.g. 2026):');
    if (!targetYear) return;
    this.monthlyMenuApi.duplicateMonthlyMenu(row.uuid, {
      target_month: parseInt(targetMonth, 10),
      target_year: parseInt(targetYear, 10),
    }).subscribe({
      next: (res) => {
        this.notification.success('Menu duplicated');
        if (res.data) { this.router.navigate(['/admin/monthly-menus', res.data.uuid, 'edit']); }
        this.loadMenus();
        this.loadStats();
      },
      error: (err) => this.notification.error(err.error?.message || 'Operation failed'),
  ...
  deleteMenu(row: MonthlyMenu): void {
    if (window.confirm(`Delete "${row.title}"? This action cannot be undone.`)) {
      this.monthlyMenuApi.deleteMonthlyMenu(row.uuid).subscribe({
  ...
  copyPreviousMonth(): void {
    const sourceMonth = window.prompt('Enter source month (1-12) to copy from:');
    if (!sourceMonth) return;
    const sourceYear = window.prompt('Enter source year (e.g. 2026):');
    if (!sourceYear) return;
    const targetMonth = window.prompt('Enter target month (1-12) to copy into:');
    if (!targetMonth) return;
    const targetYear = window.prompt('Enter target year (e.g. 2026):');
    if (!targetYear) return;
    this.monthlyMenuApi.copyPreviousMonth({
      source_month: parseInt(sourceMonth, 10),
      source_year: parseInt(sourceYear, 10),
      target_month: parseInt(targetMonth, 10),
      target_year: parseInt(targetYear, 10),
    }).subscribe({
      next: (res) => {
        this.notification.success('Previous month copied');
        if (res.data) { this.router.navigate(['/admin/monthly-menus', res.data.uuid, 'edit']); }
        this.loadMenus();
        this.loadStats();
      },
  ...
  bulkPublish(): void {
    ...
    if (window.confirm(`Publish ${ids.length} selected menus?`)) {
      let completed = 0;
      ids.forEach((menu) => {
        this.monthlyMenuApi.publishMonthlyMenu(menu.uuid).subscribe({
  ...
  bulkDelete(): void {
    ...
    if (window.confirm(`Delete ${ids.length} selected menus? This cannot be undone.`)) {
      let completed = 0;
      ids.forEach((menu) => {
        this.monthlyMenuApi.deleteMonthlyMenu(menu.uuid).subscribe({
